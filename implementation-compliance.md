...
**処理系** (implementation) または**実装**とはプ�グラムの**翻訳** (translation) と**実行** (execution) をする一連の枠組み・ソフトウェアのことである。
...
- `volatile` [glvalue](lang/cpp11/rvalue_ref_and_move_semantics.md) を通した抽象機械の�み書き操作
- 実行終了後の、ファイルに出力された内容
- 対話的な入出力装置の�み書きの順序
...
- **文化圏固有動作** (locale-specific behavior) に対しては、処理系は現地の国家・文化・言語の風習に依�した動作を行う。処理系はその動作を説明書に記述する必要がある。

これらの用語は処理系が取りうる動作の範囲を示すものであって、例えば "未定義の動作" という名前の具体的な動作がある訳ではないことに注意する。

### <a href="#rule" id="rule">規則</a>

標準規格の定める要件は、処理系 (抽象機械) に対する直接の要件と、処理系が受け入れるべきプ�グラムが満たす**規則** (rule) で構成される。
規則には幾つかの分類があるが、C++ の標準規格内では具体的な分類基準は示されていない。C言語に倣えば以下の解釈になる。

- **構文規則** (syntactic rule, syntax rule): 拡張バッカス・ナウア記法の亜種である**構文記法** (syntax notation) によって指定されるプ�グラムの構文
- **制約** (constraints): C言語において構文記法によって表現しきれない構文的な制限を文章で述べたもの。C++ には現れない
- **意味規則** (semantic rule): 構文規則と制約のどちらでもないプ�グラムに対する規則

各規則には**診�不要**<sup><a href="#note-1" id="note_ref-1">[注1]</a></sup> (no diagnostics required; 通称 NDR) や「違反すると未定義の動作になる」などの属性が明記されることがあり、
診�不要とも未定義の動作になるとも明記されない規則を**診�対象規則** (diagnosable rule) と呼ぶ。
**単一定義規則** (ODR; one definition rule) は "使用される変数・関数・クラスについてただ1つの定義を与えなければならない" という一連の規則である。

- **適格** (well-formed) とはプ�グラムが全ての構文規則・診�対象の意味規則・単一定義規則を満たすことである
- **不適格** (ill-formed) とはプ�グラムが適格でないことである

プ�グラムが規則に違反するとき、処理系はエラーメッセージまたは�告などを出力する。
この出力を総称して**診�情報** (diagnostic message) または**診�メッセージ**<sup><a href="#note-2" id="note_ref-2">[注2]</a></sup> と呼び、その内容は処理系定義である。

適合する処理系は、

- 規則を全て満たすプ�グラムをそのリソースの範囲で�しく実行 (外から見た動作を模倣) する必要がある。
- 診�対象規則に違反するプ�グラムに対して診�情報を出力する必要がある。
- 診�不要な規則に違反するプ�グラムの翻訳・実行について、標準規格によって如何なる要件もおかれない。
...
変数名の最大の長さや仮引数の最大の数など、処理系が対応する様々なプ�グラムの大きさのことを**処理系限界** (implementation limit, implementation quantity) と呼ぶ。
処理系は処理系限界についての情報を説明書に記述する必要がある。

### <a href="#relations" id="relations">規則違反と動作の包含関係</a>

規則に違反するプ�グラム・処理系依�の動作を行うプ�グラムの包含関係について以下の図に示す。

![俯瞰図](https://raw.githubusercontent.com/cpprefjp/image/master/implementation-compliance/overview.png)

- 但し、一つのプ�グラムが複数の規則に違反する場合については考慮していない。
- 構文規則の範囲は規格上曖昧なので、診�不要 (NDR) の規則を含むかどうかは解釈による。
- 不適格かつ診�不要の規則 (ill-formed; NDR) が、規格上矛盾なく定義されているかどうかは�論が分かれる。
...
- **QoI** (quality of implementation; QOI): 処理系の実装品質のこと。適合する処理系であっても処理系依�な部分で粗末な動作をするものが考えうる。
- **normative**: (ある規格の記述が) 適合する処理系に対して強制力を持つさまを表す
- **informative**: (ある規格の記述が) 強制力を持たないさまを表す

プ�グラムの性質についての慣用語

- **処理系依�** (implementation-dependent): プ�グラムの動作が処理系によって異なりうること。
  条件付き対応の機能を使用している場合や、処理系定義の動作・未規定の動作・未定義の動作・文化圏固有動作を起こす場合、処理系限界を超える場合、規則に違反している場合を含む。
- **合法** (legal)・**違法** (illegal): これらの語はプ�グラムに対しても慣用されるが、具体的な意味は明確でない。
  プ�グラムの�しさには複数の水準があるためである。
  適格、またはすべての規則を満たす、または未定義の動作を含まないなどが考えられる。
  曖昧さを避けるため、このサイトではプ�グラムに対して合法・違法という語は用いない。

## <a href="#nasal-demon" id="nasal-demon">「鼻から悪�」とプ�グラムの可搬性</a>

プ�グラムが

- 未定義の動作 (UB) を引き起こすとき、
- または診�不要 (NDR) の規則に違反している

とき、標準規格は適合する処理系に対して何らの要件も課さない。
つまり、UB または NDR 違反を含むプ�グラムに対して処理系がいかなる動作をしても規格には抵触しないということを表す。
例えば、このことで処理系が鼻から悪�を出しても、それはプ�グラムの作者の責任であり、その処理系を責めることはできない。
この冗談を**鼻から悪�** (nasal demons)<sup><a href="#cite-1" id="cite_ref-1">[1]</a></sup><sup><a href="#cite-2" id="cite_ref-2">[2]</a></sup> と呼ぶ。鼻から悪�を出す処理系は今のところ実在しないが、
実際の未定義の動作として最適化の過程で或る種の「タイムトラベル」を起こす処理系は実在する<sup><a href="#cite-3" id="cite_ref-3">[3]</a></sup>。

処理系依�のプ�グラムは、たとえ或る処理系の上で期待する動作をしたとしても、他の処理系でも�しく動作することは保証されない。
可搬なプ�グラムを書くためには、未定義の動作を引き起こさずかつ診�不要の規則に違反しないプ�グラムを書くように心懸ける必要がある。
...
- [C++er は“合法”だとか“違法”だとか言いたくて仕方がないけれど、結局どういう意味? それより適合・適格・○○動作・○○規則・診�不要いろいろの関係が謎 - Qiita](https://qiita.com/akinomyoga/items/592e5a3b8438a0c8556b)
...
- [MSC15-C. 未定義の動作に依�しない](https://www.jpcert.or.jp/sc-rules/c-msc15-c.html)

## 出典

1. <a href="#cite_ref-1" id="cite-1">**^**</a> <cite>[nasal demons](http://www.catb.org/jargon/html/N/nasal-demons.html)</cite>
2. <a href="#cite_ref-2" id="cite-2">**^**</a> <cite>[本の虫: C++0x本：鼻から悪�](https://cpplover.blogspot.jp/2010/01/c0x_14.html)</cite>
...
1. <a href="#note_ref-1" id="note-1">**^**</a> no diagnostics required: JIS C++ では一定の訳は与えられず、登場する度に異なる翻訳のされ方をしている。
  ここでは "診�不要" という語を割り当てることにする。
2. <a href="#note_ref-2" id="note-2">**^**</a> diagnostic message: JIS C++ では "診�情報" としている。
  JIS C言語 (JIS X 3010:2003) では "診�メッセージ" としている。
3. <a href="#note_ref-3" id="note-3">**^**</a> conditionally-supported constructs: C++11 で導入されたものなので、JIS C++ には対応訳は�在しない。
  ここでは "条件付き対応の構成" と訳す。
