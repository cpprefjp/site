...
| ローカルクラスと無名クラスを、テンプレート引数として使用することを許可 | |
...
| [属性構文](cpp11/attributes.md.nolink) | `[[attr]]`構文による、クラス、関数、変数の属性指定 |
...
| 依存名に対する`typename`と`template`の制限緩和 | テンプレート外でも`typename`と`template`を付けられるようになった |
