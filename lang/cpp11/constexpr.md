...
たとえば、[`std::numeric_limits`](/reference/limits/numeric_limits.md)の[`max()`](/reference/limits/numeric_limits/max.md)静的メンバ関数は、`int`型に対しては`INT_MAX`マクロの値を返すだけであるが、それが関数であるために、`INT_MAX`マクロを使うよりも遅い、という問題があった。抽象化された機能を使うより、抽象化されていない機能の方がよい、というのは、改善すべき事態だった。そのため、関数を静的に評価する仕組みが必要とされた。
