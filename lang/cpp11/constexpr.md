...
関数`square()`の先�に`constexpr`�ーワードを付けることで、その関数は、コンパイル時と実行時、両方で呼び出せる関数となる。
...
`constexpr`関数もまた、その内部で変数の書き換えはできず、戻り値の型を`void`にすることもできない。変数の書き換えをせずに、計算した結果を返す必要がある。また、`constexpr`関数には、その本体において、`return`文ひとつのみで処理を行わなければならない、という制限がある。そのため、条件分岐には`if`文の代わりに条件演算�を、ループには`while`文や`for`文の代わりに再帰を使用する必要がある。
...
- `constexpr`関数は数�的に結果が定義された計算式、および結果が値の表現範囲内である計算式を記述できる
    - つまり、整数をゼ�割りした場合は、プ�グラムは不適格となる
- `main()`関数は、`constexpr`関数として定義できない
- `throw`文は書けるが、それがコンパイル時に評価された場合には、プ�グラムは不適格となる。
...
- `constexpr`コンストラクタには、関数`try`ブ�ックは使用できない
- `constexpr`コンストラクタの本体は、空でなければならない
- 仮想基底クラスを持ってはならない
- 基底クラス、および非静的メンバ変数は、メンバ初期化�を使用して初期化しなければならない
...
`constexpr`関数では、コンパイル時にはエラー報告に`static_assert`、実行時には`throw`文や`assert()`関数マク�などを使用する必要があり、標準機能の範囲内では、コンパイル時と実行時で、エラー報告を統一的に行うことが難しい。

そういった問題を解決するために、ユーザーコミュニティで、エラー報告を統一的に行う仕組みが作られている。ここでは、その問題に取り組んでいる場所へのリンクを記載する。

- [コンパイル時/実行時両用アサート - ボレ�村上 - ENiyGmaA Code](http://boleros.hateblo.jp/entry/20130319/1363719366)


### グ�ーバル変数の初期化順
並行プ�グラミングについて考えよう。スレッド間の排他処理を行うミューテックスをグ�ーバル変数に保持したとき、ミューテックスの初期化が、スレッドの開始よりも必ず先に行われてほしい。
...
そんな状況のために、[`std::mutex`クラスのデフォルトコンストラクタ](/reference/mutex/mutex/op_constructor.md)には、`constexpr`修飾が行われている。そうすることで、その型の非�ーカル変数を作ったときに、その変数は他の任意のスレッド開始よりも先に行われることが保証される。
...
### 文�列処理
`constexpr`では、`new`演算�や`malloc()`関数が使用できないために、可変長の配列や文�列を扱うことが難しい。

C++11段階の標準ライブラリでも、そのような機能は標準では提供されていない。

こういった問題を解決するために、ユーザーコミュニティでそのような機能が作られている。ここでは、その問題に取り組んでいる場所へのリンクを記載する。

- [Sprout.String - constexpr 文�列クラス - ボレ�村上 - ENiyGmaA Code](http://boleros.hateblo.jp/entry/20110926/1318115291)

`constexpr`で文�列が扱えることにより、`printf()`のフォーマット文�列や、�規表現の間違いをコンパイル時に検出することができるようになる。


### コンパイル時再帰回数

コンパイラによっては、コンパイルオプションでコンパイル時における`constexpr`関数の再帰回数の上限を�定できる。

GCCとClangでは、`-fconstexpr-depth=`オプションで�定できる：

```
g++ main.cpp -fconstexpr-depth=1024
```

Visual C++ 2015では、`/constexpr:depth`オプションで�定できる：

```
cl.exe /constexpr:depth1024 main.cpp
```

ここの`1024`を任意の値に変更することで、再帰回数の上限を�定できる。

GCC 5.2、Clang 3.7、Visual C++ 2015時点で、3つともデフォルトは512回。


## この機能が必要になった背景・経緯
`constexpr`の主な目的は、数値型のプ�パティを取得するクラス[`std::numeric_limits`](/reference/limits/numeric_limits.md)の、プ�パティ取得の関数を定数式にすることである。

たとえば、[`std::numeric_limits`](/reference/limits/numeric_limits.md)の[`max()`](/reference/limits/numeric_limits/max.md)静的メンバ関数は、`int`型に対しては[`INT_MAX`](/reference/climits/int_max.md)マク�の値を返すだけであるが、それが関数であるために、[`INT_MAX`](/reference/climits/int_max.md)マク�と違って定数として扱えない、という問題があった。抽象化された機能を使うより、抽象化されていない機能の方がよい、というのは、改善すべき事態だった。そのため、関数を静的に評価する仕組みが必要とされた。

また、`constexpr`は、値を計算するテンプレートメタプ�グラムを置き換えて使用できる。テンプレートメタプ�グラミングでは、非型テンプレートパラメータによって整数型の値をコンパイル時に計算することはできた。しかし、浮動小数点数型の値や、その他多くの値に関する計算が難しく、構文もまた通常の関数とはかけ離れていた(浮動小数点数型の値の計算は、分数形式にすれば、できることはできる)。値をコンパイル時に計算するためには、今後はテンプレートメタプ�グラミングよりも`constexpr`を積極的に使用していくとよいだろう。
