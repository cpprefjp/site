...
なお、C++14 では特定の文脈において式のかわりに `auto` �ーワードを使用する [`decltype(auto)`](../cpp14/decltype_auto.md) が追加されている。
...
- `e` が丸括弧で囲まれていない�別�の場合、あるいは、丸括弧で囲まれていないクラスメンバアクセスの場合、`decltype(e)` は `e` で指定された実体の型である。  
	もしそのような実体が無いか、あるいは、`e` がオーバー�ードされた複数の関数を指す場合、プ�グラムは不適格である。
...
ラムダ式の関数本体で、外部の自動変数に対して `decltype` を余分な丸括弧付きで使用した場合、ク�ージャ型のメンバ変数に対する `decltype` とみなされる。
...
### new 式の型、�ャスト、および、明示的デストラクタ呼び出しの一部としての使用例
```cpp example
#include <iostream>
...
int main()
{
  ...
  std::cout << "destruct\n";
  auto sp = static_cast<decltype(f())*>(p);     // デストラクタ呼び出しのため、S 型へのポインタに�ャスト
...
struct S {
  int i;
  S(int i) : i(i) {}
  operator decltype(i)() { return i; }        // int 型への変換演算�
};
...
C++03 まででは、このような関数テンプレートの戻り値型をあらゆるケースで�確に表現することは不可能だった。
...
GCC や Clang には `decltype` が導入される前から、同様の機能を持つ `typeof` が非標準の拡張機能として�在している。  
`decltype` もこれらの拡張機能を参考にしているが、大きく以下の 3 点において `typeof` とは異なる。  

- 参照の扱い。  
	`decltype` はオペランドの型にある参照を保�するが、`typeof` はオペランドの型から参照を外す。  
	ただし、上記の仕様に書かれているように `decltype` もオペランドが単なる�別�やクラスメンバアクセスだった場合には、当該�別�やメンバがたとえ lvalue であっても左辺値参照とはならない。  
