...
これを使用することによって、複数の翻訳単位で同じテンプレートのインスタンス化が行われることを抑制できる。それによって、コンパイル速度が向上すること、および実行ファイルのサイズが膨れ上がるのを抑える効果が期待できる。
...
##備考
バイナリサイズの肥大化については、コンパイラの「リンク時最適化 (Link Time Optimization, LTO)」によっても抑制を期待できる。
