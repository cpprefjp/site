...
## 概要
`override`は仮想メンバ関数のオーバーライドを明示的に宣言する�ーワードである。

`final`は、これ以上の継承をさせないことを明示的に宣言、または派生クラスでの仮想メンバ関数のオーバーライドをさせないことを明示的に宣言する�ーワードである。
...
- `override`と`final`はコンテ�スト依��ーワードである。
    - �ーワード（例えば`return`や`new`など）は常に�ーワードとして扱われるが、`override`と`final`は特定の場所に書かれたときだけ�ーワードとして扱われる。
    - 簡単に言えば、`final`はクラス名の後かメンバ関数名の後に限り、`override`はメンバ関数名の後に限って、�ーワードとして扱われる。


## 詳細な仕様
�確ではないが、クラス名の宣言（class-head）は下記の文法となる：
...
- class-key：クラスの�ーワード指定、`class`, `struct`, `union`のどれかが書ける
...
- オーバーライドしてはならない関数を、誤ってオーバーライドする間違いを検知する。
    ...
    - C++11では`B::f()`を`final`と宣言することで、オーバーライドの禁�を明示でき、意図しないオーバーライドをした場合は文法違反として検知できる。
...
- なぜ�ーワードではなく、コンテ�スト依��ーワードが選ばれたか？（N3163の提案）
    - `final`や`override`のような簡潔で良い名前が使えて、なおかつ、過去との互換性を�持できるためである。
    - `final`や`override`を�ーワードとした場合`final`や`override`を関数名や変数名として使っていたコードがC++11では文法違反となってしまう。しかしコンテ�スト依��ーワードであれば文法違反にならない。

例えば下記のようなコードは`final`や`override`を�ーワードとすると文法違反となる：
...
- N1827にて`new`�ーワードと明示的にオーバーライドを宣言する記法が提案された。
...
- N2108にて`new virtual`、`explicit`�ーワードの追加と、`virtual`�ーワードの意味の変更が提案された。
...
- N2365では`[[check_names]]`、`[[new]]`、`[[hiding]]`属性（attribute）の追加と、`virtual`�ーワードの意味の変更が提案された。
    ...
    - `[[check_names]]`を指定しなければ、C++03までと同じ動作をする。
        ...
        - オーバーライドのチェックの有無を選べるようにすることで、C++03で�しかったコードを破壊しないように配慮された。
    - `[[new]]`は新たな仮想メンバ関数を宣言し、オーバーライドしていれば文法違反となる。
        - N2108の`new virtual`�ーワードとほぼ同じアイデアである。
...
- N2852では`[[override]]`、`[[hiding]]`、`[[check_names]]`属性の追加が提案された。
    - `[[override]]`は仮想メンバ関数のオーバーライドを宣言し、オーバーライドできていなければ文法違反となる。
        - N2365では`virtual`�ーワードの意味を変更して同様の機能を実現していたが、N2852では`virtual`の意味は従来通りに留め、`[[override]]`に役割を�った。
...
- N2928にて`[[check_names]]`属性は`[[base_check]]`属性に名前が変えられた。
    ...
    - なぜ名前を変えたのかわからなかった。加�いただけると嬉しい。


- N3151では投票の結果、属性（attributes）は望ましくないとされ、残った2つの選択肢である�ーワードとコンテ�スト依��ーワードのどちらが良いか、検討が行われた。
...
注：表�のSFやWFという単語は、SF: Strongly Favor, WF: Weakly Favor, WA: Weakly Against, SA: Strongly Againstの略である。

コンテ�スト依��ーワード（この例では`hides_name`という名前になっている）にはDaveed Vandevoordeが指摘した問題がある。この問題は`[[hiding]]`属性や`strictdecl`�ーワードならば発生しない：
...
- N3151では結論として�ーワードを提案した。
    - �ーワードの場合はユーザが混乱するような変な名前を避け、なおかつ過去との互換性をできるだけ保つ名前を選ぶ必要があった。
    - そのため、既�のコードで使われていない、つまり既�のコードが文法違反となるケースが少ない名前が調査された。
- 最終的に下記の名前が提案された。
...
- N3163ではコンテ�スト依��ーワードを使うことの利点と、`base_check`、`hiding`、`override`、`final`コンテ�スト依��ーワードの追加が提案された。


- N3206では`[[base_check]]`、`[[hiding]]`、`[[override]]`、`[[final]]`属性を削除し、`explicit`、`new`、`override`、`final`コンテ�スト依��ーワードの追加が提案された。
...
- N3234にある通り、`new`コンテ�スト依��ーワードはメンバ関数名の後にしか書けないため、メンバ関数をメンバ変数で隠した場合に`new`を宣言したいが文法上不可能な問題がある。
...
- N3234では`new`、`explicit`コンテ�スト依��ーワードの削除が提案された。
    - `explicit`だけ残すと、メンバ関数以外でメンバ名のhidingを起こす既�のコードが破壊されるため、`new`とともに削除された。
    - N3272で提案が取り込まれた。


- 以上の経緯を経てC++11では、`override`と`final`コンテ�スト依��ーワードの追加のみが行われた。
