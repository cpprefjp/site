...
- 列挙型には、型名の後ろにコ�ン `:` 区切りで、基底の整数型を指定できる。基底型として指定した整数型のCV修飾は無視される
    ...
    - 列挙�の値が基底型の範囲に収まらない場合、プ�グラムは不適格となる
...
- スコープを持たない列挙型に基底型を指定しない場合、その列挙型の全ての列挙�の値を表現できる整数型のいずれかが基底型となる（どの整数型となるかは実装定義だが、すべての列挙�の値が`int`もしくは`unsigned int`に収まる限り`int`より大きな型にはならない）
- スコープを持たない列挙型の列挙�が空である場合、値`0`ひとつを列挙�として持つ
- 2つの列挙型の基底型が同じである場合、それらの列挙型は互換性のあるメモリレイアウトを持つ
- 列挙�に値を指定しない場合、最初の列挙�には定数値`0`が�定される。2つ目以降の列挙�は、ひとつ前の値を`1`進めた値を持つ
...
ECMA規格になっているC++/CLIが、現在のスコープを持つ列挙型と�価の機能を持っていたために、その経験を標準C++に取り入れることとなった。
