...
### 複数個のreturn文
複数の`return`文がある場合には、すべての`return`文に対してそれぞれ戻り値の型を推論し、それらが全て同じ型であればその型が戻り値の型となる。

推論した型が全て同じにならない場合、プログラムは不適格となる。それらの型に共通の型が存在するかどうかは関係しない。

```cpp
// xもcxもintと推論される (autoの推論では参照とトップレベルのconstが取り除かれる)
// ため、戻り値の型はint
auto f()
{
  int x = 3;
