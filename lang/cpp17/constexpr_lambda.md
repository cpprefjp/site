...
また、制限を満たすラムダ式が生成するク�ージャオブジェクトがリテラルとして扱われるようになった。

## 仕様

ラムダ式の�ャプチャが全てコンパイル時定数とき、そのラムダ式をconstexprの文脈で使うことができる。
...
ラムダ式が生成するク�ージャオブジェクトは、ラムダ式の�ャプチャが全てコンパイル時定数のとき、リテラル型として扱われる。
...
以下のような記述を許可してしまうとあらゆる文や式が実体化に成功するかをチェックでてしまうため、禁�されている。
...
[機能テストマク�](../../lang/cpp17/feature_test_macros.md)は__cpp_constexprで、値は201603。

（__cpp_constexprマク�の値は、C++11では200704、C++14では201304だった。）
