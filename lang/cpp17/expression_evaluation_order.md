...
- 左から右に評価される。
    - メンバへのポインタ演算� (`.*`, `->*`)
    - 関数呼び出し演算�、および`T(...)`による初期化
    - 添え�演算�
    - シフト演算�
- 右から左に評価される。
    - 代入演算�
    - 複合代入演算�（代入と演算を同時に行う`+=`, `-=`, `|=`などのこと）
- オーバー�ードされた演算�の場合、同様の組み込み演算�の評価順序によって決定される。
    - オーバー�ードされた演算�の実際の動作はメンバ関数呼び出しだが、わざとメンバ関数呼び出しの規則を適用しない。
- `new`式（メモリ確保を初期化�の評価順序の前に規定した）

3番目の規則については、
代入演算�`operator=`をオーバー�ードした場合を考えるとわかりやすいと思う。
代入演算�は右から左の順、つまりa, bの順で評価される。
...
Hogeクラスの代入演算�をオーバー�ードすると、
`b = a;`は`b.operator=(a);`となりメンバ関数呼び出しと�価になる。
このとき3番目の規則が�在しなかったとすると、メンバ関数呼び出しの規則が適用される。
...
プ�グラムの見た目は全く同じ代入式にも関わらず、
代入演算�オーバー�ードの有無によって評価順序が逆になってしまう。
このような振る舞いはプ�グラマを混乱させるだけだろう。
3番目の規則は「演算�オーバー�ードにわざとメンバ関数呼び出しの規則を当てはめない」ことで、
演算�オーバー�ードの有無に関わらず自然な動作を実現するための規則である。
...
C++規格の�定以来、式の評価順序は厳密に定められていなかった。
...
シフト演算�は左結合なので`(((std::cout << f) << g) << h)`の順に評価されそうだが、
残念ながらそうならない。
...
`std::cout`はシフト演算�オーバー�ードを行っているため、
...
動作結果が不定となってしまう問題がある。
この問題はプ�グラミング作法が悪いのではなく、
C++規格が現代のプ�グラミング作法に合わなくなっていることが原因である。
C/C++規格の制定から約30年の時を経て、C++17で是�されることとなった。

C++17では全ての式の評価順を厳密に定めることはあえて避けている。
C++の既�のプ�グラムを壊すこと無く、なおかつ、
広く用いられているプ�グラミング手法が不定な動作にならないよう、
