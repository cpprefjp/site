...
関数の引数リスト内の式（上記例で言えばb1, b2, b3）の評価順序は未定義である。
...
int main() {
  std::map<int, int> m;
  m[0] = m.size(); // C++14 では m が {{0, 0}} か {{0, 1}} のどちらになるか未定義
}
```
...
広く用いられているプログラミング手法が未定義の動作にならないよう、
