...
2を底とする、指数表記の十�進浮動小数点数リテラルがサポートされた。

## 仕様

```
十�進プレフィックス 仮数部 指数部 サフィックス
```
...
十�進浮動小数点数リテラルには [桁区切り文�](/lang/cpp14/digit_separators.md)を使用できる。

十�進表記で浮動小数点数を出力するには、
`std::ostream` では [`std::hexfloat`](/reference/ios/hexfloat.md)、
`std::printf` では `%a`/`%A` を用いる。

[`std::strtod`](/reference/string/stod.md) は十�進表記で浮動小数点数の入力を受け取る
...
{
  std::cout << 0xABCp-3f << '\n';
  double dbl_min = 0x1.0p-1'022; // 1 * 2^-1022 (double の�の�規化数の最小値)
...
## この機能が必要になった背景・経緯
十�進リテラルによってある浮動小数点数を�確に表現することができるようになった。

## 備考

### C
C99 で十�進浮動小数リテラルが採用された。

### <a id="notes-iostreams"></a>iostream の問題
[`std::strtod`](/reference/string/stod.md) は `pP` を認�しないため、十�進表記で浮動小数点数を受け取って適切に変換することができない
