...
これまで、`opeartor ++`の定義は、bool型のときはtrueに変更する、`opeartor --`の定義はbool型を除く、というように例外規定されていた(§ 8.2.6 / § 8.3.2)。  
C++17ではこれらが削除され、`opeartor ++`の定義(§ 8.2.6 / § 8.3.2)に、bool型を除く、という例外規定が追加された。

前置の`operator ++`と`operator +=`の呼び出し(例えば`++a`と`a+=1`)が等価にならない例に、bool型の場合、という文面があったが、C++17で削除された(§ 8)。

また、組み込みのoperatorのリストの`operator ++`に関する文面に、bool型を除く、という例外規定が追加された(§ 16.6)。
