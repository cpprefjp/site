...
## 概要
新しく三方比較演算�`<=>`が導入されることにより、順序付けと同値比較の6つの関係演算�（`<`, `<=`, `>`, `>=`, `==`, `!=`）を容易に実装することができるようになる。
...
struct C {
  ...
  //<=>をpublicで定義しておくことで、その他の演算�が導出される
...
この様に、あるクラスに対して三方比較演算�`<=>`を定義しておくことで最大6つの比較演算�を導出し使用することができる。  
そして、そのような`<=>`は`default`実装で十分ならば実装を省略できる。

この様な三方比較の事を一貫比較（Consistent comparison）と言い、この演算�は三方比較演算�（Three-way comparison operator）と呼ぶ。また、演算�の見た目から宇宙船演算�（Spaceship operator）と呼ばれることもある。

この様に、三方比較演算�を用いれば比較演算�の定義が非常に容易になるため[`std::rel_ops`](/reference/utility/rel_ops.md)はその役割をほとんど失い、非推奨となった。

## 仕様

### 三方比較
ある型の値`a, b`についての`a <=> b`による比較の結果は単純な`bool`ではなく、未満・�しい・超える、という3つの関係を同時に表す値を返す。  
...
なお、三方比較演算�の戻り値の`0`リテラル以外との比較は未定義動作とされる。`1`だったり`0.0`であってはならない。
...
三方比較演算�の戻り値型は`int`などの整数型ではなく、比較カテゴリ型と呼ばれる専用の型である。  
これは、比較対象となる型の満たしている同値や順序の関係についてを専用の型によって表明し、コンセプト�の機構によってその性質に応じた適切な処理へのディスパッチを行うことを出来るようにするためである（例えば、以下で述べる比較カテゴリ型によって導出する演算�を変化させるのに利用されている）。

以下の5つの比較カテゴリ型が提供される。

|比較カテゴリ型|対応する数�的な関係|導出される演算�|
|:---|:---:|:---:|
...
表にあるように3つの比較カテゴリ型はそれぞれ数�的な2項関係（順序関係）の一つと対応している。

三方比較演算�による比較の結果となる値は、これら比較カテゴリ型のいずれかの`prvalue`オブジェクトとなる。  
全てのカテゴリにおいてそのようなオブジェクトの`0`リテラル以外との比較は未定義動作となる。

これらの比較カテゴリ型は新しく追加される[`<compare>`](/reference/compare.md)ヘッダにて定義されるが、`<=>`をコード�で使用したとしても自動でインクルードされないため、`<=>`の使用も含めて比較カテゴリ型を利用する際は`<compare>`を明示的にインクルードする必要がある。

#### 比較カテゴリ間の順序関係

各比較カテゴリ型はその条件の強いものから弱いものへの暗黙変換が定義される。この方向は各カテゴリに対応する数�的な関係の包含関係によって定義されている。  
...
クラス型に対するdefaultな三方比較演算�の様に複数の型が参加する三方比較の結果の比較カテゴリは、比較に参加するすべての型の`<=>`による比較の結果となるカテゴリ型から共通して変換できる最も強い型となる。そのような型を共通比較カテゴリ型（common comparison category type）と呼ぶ。
...
### operator==
`<=>`を利用する事で最大6つの比較演算�が導出されると説明したが、実際には同値比較演算�（`== !=`）は`<=>`から導出されず、`==`は定義されている必要があり`!=`は`==`から導出される。  
...
このため、異種型間比較や特殊な比較を実装するために`<=>`を独自に定義する場合、6×2個の比較演算�全てを導出するためには`==`も独自に定義しなければならない。

```cpp
//<=>を独自実装し==を実装していない型、４種類の比較演算�による比較が可能
...
### 演算�の導出とオーバー�ード候補
`<=>`及び`==`から導出される演算�は暗黙的に宣言され実装されているわけではなく、それらの演算�を呼び出した際のオーバー�ード候補に、`<=> ==`を利用して生成した候補を入れることによって導出される。  
このために、導出された比較演算�のアドレスを取ることは出来ない。

詳細な手順は以下のようになる。

任意の演算�`@`を任意の型`T1, T2`のオブジェクト`a, b`に対して`a @ b`のように呼び出したとき

1. オーバー�ード候補に、`a @ b`を加える
2. `@`が三方比較演算�ならば、そのオーバー�ード候補に`<=>`を使って生成した逆順の式`b <=> a`を加える
3. `@`が関係演算�（`< <= > >=`）ならば、そのオーバー�ード候補に`<=>`を使って生成した2種類の式`a <=> b, b <=> a`を加える
4. `@`が`!=`ならば、そのオーバー�ード候補に`==`を使って生成した2種類の式`a == b, b == a`を加える
5. `@`が`==`ならば、そのオーバー�ード候補に`==`を使って生成した逆順の式`b == a`を加える
6. オーバー�ード解決では使用可能なメンバ・非メンバ・組み込みの`@ <=> ==`が考慮され、`@`→�順の式→逆順の式 の順で優先される。また、選択された演算�に対して式の生成は行われない。
7. それらの候補からのオーバー�ード解決の結果生成された候補が選択された場合、その候補に応じて以下のように最終的な式を生成する
    - `@`が三方比較演算�ならば、`0 <=> (b <=> a)`
    - `@`が関係演算�（`< <= > >=`）ならば、`(a <=> b) @ 0`もしくは`0 @ (b <=> a)`
    - `@`が`!=`ならば、`!(a == b)`もしくは`!(b == a)`
8. 元の呼び出し`a @ b`を生成された式で書き換える（生成された式を元の呼び出し`a @ b`として実行する）

結果、各演算�を呼び出したときに考慮されるオーバー�ード候補は以下のようになる。  

| 呼び出す演算� `a @ b` | オーバー�ード候補                                                           |
| :-------------: | :-----------------------------------------------------------------: |
| `a <=> b`       | `a <=> b` <br/> `0 <=> (b <=> a)`                                    |
| `a == b`        | `a == b`<br/>`(b == a)`                               |
...
この様に、異種型間比較においても片方の`<=> ==`を定義しておけばその引数順を逆にした演算�も生成され、演算�の対称性が自動で補完される。
...
`== <=>`が使用可能ではない（定義されてない、曖昧、アクセスできない、削除されている）場合は、オーバー�ード解決の過程で候補から外されコンパイルエラーとはならない（上記手順6以前のエラーはコンパイルエラーとならない）。

### default比較
ここまでにも説明せずに登場していたが、あるクラス型に対する`<=>`および`==`演算�は`default`指定することができる。  
...
そして、このようなdefault宣言はその定義が`constexpr`関数の要件を満たしていれば暗黙的に`constexpr`指定され、呼び出す演算�が全て`noexcept`であるならば暗黙的に`noexcept`である（これらの指定は明示的に指定しておくこともできる）。

`default`指定された三方比較演算�の戻り値型は基底クラス及び全メンバの`<=>`の結果型の共通比較カテゴリ型となるが、その型が`void`である場合は暗黙的に`delete`される。  
その際、暗黙宣言される`==`演算�は定義可能（比較に参加するすべての型について`==`の呼び出しが適格）ならば`default`で宣言される。


#### default実装
default宣言された`<=> ==`演算�はその基底クラスと非静的メンバを宣言順に比較していくことで実装される。

その手順は以下のようになる（演算�`@`は`<=> ==`のどちらかとする）。
...
この時、使用可能な`<=> ==`演算�が見つからない場合、およびメンバに参照型を持つか匿名共用体を含む、もしくはその型が共用体である場合は宣言された全ての比較演算�のdefault宣言は暗黙的にdeleteされる（下記のその他演算�のdefault宣言も含む）。

#### default実装における<=>の合成

`<=>`のdefault比較実装はメンバおよび基底クラスに`<=>`を持たない型があると暗黙deleteされる。しかし、これでは現在使われている多くの`<=>`を持たない型をメンバに持つ際にdefaultの`<=>`を提供できない。  
しかしその場合にも、`<=>`のdefault宣言に戻り値型を指定した上で`<=>`を持たないメンバ（基底）型が`< ==`の2つの演算�を実装していれば、それらを元にして`<=>`を合成した上でdefault実装を行うことができる。
...
struct newer {
  ...
  //legacyの比較に関しては指定した戻り値型とlegacyの持つ比較演算�< ==を用いて実装、使用可能
...
|条件|合成された`<=>`の式|
|:-------------|:-------------|
|`a <=> b`のオーバー�ード解決で使用可能な`<=>`が見つかる|`static_cast<R>(a <=> b);`|
...
この合成において使用される`< ==`演算�の戻り値型の妥当性はチェックされない。仮に`bool`ではなかったとしても、合成された式においてコンパイルエラーが発生しなければ`<=>`の合成はつつがなく行われる。  
また、合成された`<=>`が定義されない（上記条件のいずれも当てはまらない）場合はdefault指定の`<=>`は暗黙にdeleteされる。

#### その他の比較演算�のdefault宣言
`<=> ==`だけでなく、残りの比較演算�もdefault指定で宣言することができる。その有効な宣言は`<=> ==`に従う。

そのようなdefault実装はオーバー�ード解決時に生成される式と同様の式を使って`<=> ==`から実装される。

`<=> ==`演算�が使用可能ではない場合や`<=>`の戻り値型が対象の演算�を生成できない場合、`==`の戻り値型が`bool`ではない場合はそのdefault宣言は暗黙的に`delete`される（オーバー�ード候補生成時はコンパイルエラーとなる場合でも単にdeleteされる）。
...
これは、比較演算�のアドレスを取りたいときに使用する。

### 組み込み型の三方比較

三方比較演算�は`void`、`std::nullptr_t`、関数/メンバポインタ、および参照型を除く組み込みの型に対して、組み込みの物が提供される。  
...
|[`浮動小数点型`](/reference/type_traits/is_floating_point.md)|`std::partial_ordering`|縮小変換が行われる場合は比較不可<br/>`NaN`や`±0.0`の�在のため半順序|
...
三方比較演算�による比較は従来の比較演算�の挙動とは異なるところがある（より安全な比較となっている）。  
それに伴って、いくつかの比較演算�の挙動が修�された（C++20では非推奨とされ、削除されてはいない）。

|比較するペア|C++17までの従来演算�での比較の可否|`<=>`での比較の可否|C++20からの従来演算�での比較の可否|
|:-------------|:-------------|:-------------|:-------------|
|符号なし整数型と符号付整数型|〇|×<br/>ただし定数式で符号付きオペランドが�の値に評価されれば可能|〇（従来通り）|
|列挙型と算術型|〇<br/>例えば、列挙型と浮動小数点型の比較が可能|△<br/>スコープ無し列挙型と整数型のみ可能|△<br/>列挙型と浮動小数点型間比較は不可<br/>それ以外は従来通り|
|異なる列挙型間|〇|×|×|
|配列同士|△<br/>先�要素へのポインタの比較になる|×|×|
...
## この機能が必要になった背景・経緯
C++17以前の例に示したように、従来のC++における比較演算�の実装は煩雑でボイラープレートの様なコードを大量にコピーアンドペーストすることになる。さらに、異種型間比較を加えるとその対称性のために同じ比較について引数型を逆にしたものを用意しなければならないため、そのようなボイラープレートは更に倍になることになる。  
この問題は以前から認�されており、比較演算�が実際には`< ==`の2つから残りのすべてを導出できることを利用して実装を簡易にする、[Boost Operators Library](https://boostjp.github.io/tips/operators.html)や[`std::rel_ops`](/reference/utility/rel_ops.md)�が提供されていた。しかし、これを用いても異種型間比較におけるボイラープレートを完全に取り除くことは出来ない。

また、比較演算�の実装に伴う別の問題として、クラスの全メンバが参加するような構造的な比較を提供する際にも、メンバの列挙方法が無いために全てのメンバを`<`や`==`で繋いで回るコードを書くことを強いられていた。  
比較演算�実装の多くの場合はこのような構造的な比較を提供すれば十分であり、その場合はクラスによってメンバ名�が違えど全メンバの辞書式比較を行うという点に変わりはない。

これらの問題の解決を言語機能によって提供するために、三方比較演算�が導入された。

上で示したように、任意のクラス型に対する比較演算�の実装は`<=>`を1つ定義するだけで完結する。その比較が構造的なものであるならば、`default`指定することで定義を書く必要すらない。  
そして、`default`指定された`<=>`と`==`は基底クラス及び全メンバの宣言順の辞書式比較を行う。  
異種型間比較においても、1つの引数順の`<=>`と`==`の2つを定義することで残りの11個の比較演算�を導出することができる。

## 検討されたほかの選択肢

### `<=> != ==` : 同値比較の分離

当初の三方比較演算�から導出される演算�は同値比較（`== !=`）のものも含めた最大6つであった。しかし、同値比較なら比較についての処理を�絡評価できる場合に、`<=>`を用いて`== !=`を導出すると�絡評価が行われず非効率になるケースがあったため、`<=>`から`==`を切り離し、`!=`は`==`から導出するように変更された。
...
これは、保持する要素に対する辞書式比較を行う実装で既�の比較演算�と�価の処理である。  
実際の比較は[`compare_three_way`](/reference/compare/compare_three_way.md)に移�しているが、これは`T`に`<=>`があればそれを利用し無ければ`<`と`==`を使って比較を行う関数である（C++20より利用可能）。
...
ところで、当初の仕様では`== !=`も三方比較演算�から導出されており、その際に生成する式は以下のように規定されていた。
...
この`has_vector`クラスは以前の仕様の下でも`<=>`を利用して6つの比較演算�による比較が可能である。  
...
また、この様な新たなボイラープレートのコピーアンドペーストが必要になるかどうかは、クラスのメンバ型の全ての`== !=`演算�の実装を再帰的に辿り判�する必要がある。

このような新たな煩雑さの導入は当初の一貫比較が目指した方向性とは真逆であり、RustやHaskell�の他言語においても同値比較と順序付けの演算�は区別されたうえで自動実装が行われていることから、C++においても同様に`<=>`から`== !=`が切り離されることとなった。
...
これらの型を返す`<=>`による比較においては、`== !=`演算�のみが（`==`演算�によって）導出されていた。

|比較カテゴリ型|対応する数�的な関係|導出される演算�|
|:---|:---:|:---:|
|`weak_equality`|同値関係|`== !=`|
|`strong_equality`|相�関係：最も細かい同値関係|`== !=`|
...
- C++20にて承認された提案文書
    ...
    7. [P1630R1 Spaceship needs a tune-up](http://wg21.link/p1630)
        - `==`の戻り値型を`bool`限定にするなど、一貫比較仕様全般の細かいバグ修�
