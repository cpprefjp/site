...
export {
  ...
  static_assert(std::is_trivially_copyable_v<Foo>); // エクスポートされない
  using namespase std;                              // エクスポートされない
}
```
...
モジュールをインポートすると、そのモジュールがエクスポートする名前が名前探索の候補に挙がるようになる。
...
C++20では、インポータブルヘッダーに対する`#include`は自動的にインポートに置き換わる。この処理はプリプロセッサで行われる。
...
* ヘッダーユニット内の宣言はすべてグローバルモジュールに属し、暗黙的にエクスポートされる。
