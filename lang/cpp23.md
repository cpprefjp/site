...
- Rangeから任意のコンテナに変換するRangeアダプタ[`std::ranges::to()`](/reference/ranges/to.md.nolink)を追加
...
### メモリ
- [`<memory>`](/reference/memory.md)に、レガシーC関数からスマートポインタへの直接出力をサポートする、スマートポインタアダプタ[`std::out_ptr`](/reference/memory/out_ptr.md)と[`std::inout_ptr`](/reference/memory/inout_ptr.md.nolink)を追加
