...
## 効果
- (1) : `any(rhs).`[`swap`](swap.md)`(*this)`と�価。この効果自体では例外を送出しない
- (2) : `any(`[`std::move`](/reference/utility/move.md)`(rhs)).`[`swap`](swap.md)`(*this)`と�価。この効果自体では例外を送出しない。`*this`の状態は、この関数を呼び出す前の`rhs`と�価の状態となり、`rhs`は有効だが未規定の状態を持つようになる
