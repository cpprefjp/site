...
- 実行時の状況によらず常に�ックフリーに振る舞えるかを表す`atomic::is_always_lock_free`メンバ定数 (C++17) が定義されているが、こちらは動的リンクライブラリのバージョンアップなどで、将来的に�ックフリーに振る舞う可能性がある場合などに値が変動する
- また、コンパイル時の型レベルでの�ックフリー保証をあきらめることで、適�なアライメントをもつアトミック変数のみを対象とした効率的実装を選択できる場合がある
