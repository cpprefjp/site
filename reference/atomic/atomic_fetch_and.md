...
namespace std {
  ...
  template <class T>
  T atomic_fetch_and(volatile atomic<T>* object, T operand) noexcept;
...
[memory_order_seq_cst](https://sites.google.com/site/cpprefjp/reference/atomic/memory_order)のメモリオーダーにしたがって、現在の値に`operandをANDした値`でアトミックに置き換える
