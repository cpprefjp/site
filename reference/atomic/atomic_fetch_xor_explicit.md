...
namespace std {
  ...
  template <class T>
  T atomic_fetch_xor_explicit(volatile atomic<T>* object, T operand, memory_order order) noexcept;

  template <class T>
  T atomic_fetch_xor_explicit(atomic<T>* object, T operand, memory_order order) noexcept;
...
`order`で指定されたメモリオーダーにしたがって、現在の値に`operandを`XOR`した値`でアトミックに置き換える
...
```
* atomic_fetch_xor_explicit[color ff0000]
