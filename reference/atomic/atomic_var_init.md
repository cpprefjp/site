...
## 効果
このマク�は、静的ストレージを持つアトミック変数を`value`で初期化するためのトークン列へと展開される(この操作は�ックの初期化を必要とするかもしれない)。変数の初期化�に並行アクセスされた場合、それがアトミックな操作であったとしてもデータ競合を引き起こす。


## 備考
このマク�は、`atomic`オブジェクトに対しては`atomic<int> x(value);`と書くのと�価である。C言語との互換性のために�在している。
