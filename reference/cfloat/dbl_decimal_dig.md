...
## 概要
`double`型の数値を10進数で�確に表すのに必要な有効数�の桁数を表すマク�。  
より�確には、`double`型のあらゆる数値を、$n$ 桁の10進数に変換し、また元に戻した場合に値が変わらないような最小の整数値 $n$ を表すマク�。  
以下の式で表される。
...
[`std::numeric_limits`](/reference/limits/numeric_limits.md)`<double>::`[`max_digits10`](/reference/limits/numeric_limits/max_digits10.md) と�しい。
