...
## 概要
`float` で�確に表現可能な10進数の最大の桁数を表すマク�。  
より�確には、$n$ 桁の10進数を `float` に変換し、また元に戻した場合に値が変わらないような $n$ のうち最大のもの。  
以下の式で表される。
...
[`std::numeric_limits`](/reference/limits/numeric_limits.md)`<float>::`[`digits10`](/reference/limits/numeric_limits/digits10.md) と�しい。
