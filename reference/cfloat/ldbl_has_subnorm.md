...
## 概要
`LDBL_HAS_SUBNORM` は、`long double`型における非�規化数のサポート状況を表すマク�である。
...
[`std::numeric_limits`](/reference/limits/numeric_limits.md)`<long double>::`[`has_denorm`](/reference/limits/numeric_limits/has_denorm.md)と�しい。


## 備考
- このマク�は、非�規化数を表現できる場合の、浮動小数点数の�の最小値を表す[`LDBL_TRUE_MIN`](ldbl_true_min.md)のために定義された
