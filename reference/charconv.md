...
`<charconv>`ヘッダでは、�ケール依�なし、フォーマット解析なしの高速な文�列・数値変換を提供する。

これらの変換は以下のような特徴がある。

- フォーマットは引数で指定し、入力値のフォーマットを実行時に解析することはない
- 動的にメモリ確保を行わない
- �ケールを考慮しない（C�ケール (POSIX�ケール) 固定）
- バッファオーバーランしない
...
| 名前            | 説明           | 対応バージョン |
|-----------------|----------------|----------------|
| [`to_chars`](charconv/to_chars.md) | 数値 → 文�列の変換 | C++17 |
| [`from_chars`](charconv/from_chars.md) | 文�列 → 数値の変換 | C++17 |
...
XMLやJSONに代表されるテ�ストベースのデータを利用するケースが増えており、C++においてそれを利用するケースも増加している。
それらは多くの場合コンピュータによって生成されコンピュータによって�まれるため国際化対応の必要がなく、生成する側でも�む側でも�ケールに非依�であり、考慮するべきではない。  
そして、これらのデータは多くの場合通信に利用されており高いスループットを要求される。

しかし、C++17未満の環境で利用可能であった文�列変換法はいずれも�ケールに依�しており�ケール非依�な変換が提供されていなかったうえに、いずれの方法も動的なフォーマットの解析・指定や動的メモリ確保、エラー報告の不足�何かしら使いにくい所があった。  
...
また、浮動小数点数を10進文�列→2進浮動小数点数→10進文�列と変換した時に最初と最後の文�列が一致する保証がある変換方法も提供されていなかった。

各変換方法とその問題点

| 関数                       | 欠点                                  |
| -------------------------- | ------------------------------------- |
| `sprintf`                  | �ケール依�、フォーマットの動的解析、バッファオーバーランの危険性     |
| `snprintf`                 | �ケール依�、フォーマットの動的解析                    |
| `sscanf`                   | �ケール依�、フォーマットの動的解析                    |
| `atol`                     | �ケール依�、エラー報告の不足                       |
| `strtol`                   | �ケール依�、先�ホワイトスペースの無視、接�辞(0x�)を考慮する               |
| `strstream`                | �ケール依�、先�ホワイトスペースの無視                  |
| `stringstream`             | �ケール依�、先�ホワイトスペースの無視、動的メモリ確保          |
| `num_put / num_get facets` | �ケール依�、仮想関数                           |
| `to_string`                | �ケール依�、動的メモリ確保                        |
| `stoi`�                   | �ケール依�、動的メモリ確保、先�ホワイトスペースの無視、接�辞(0x�)を考慮する、例外の送出 |


これらの問題がなく高速な文�列・数値変換を提供するために、本ヘッダ及び`to_chars, from_chars`関数が導入された。各関数の特徴は冒�及び個別のページを参照のこと。
...
## 参照
- [C++1z �ケール依�なし、フォーマット解析なしの高速な文�列・数値変換 - Faith and Brave - C++で遊ぼう](https://faithandbrave.hateblo.jp/entry/2016/08/24/181540)
