...
## 概要
`system_clock`は、システム時間を表現するためのク�ックである。
...
### うるう秒の扱い
このク�ックでは、うるう秒が除外されて (うるう秒が挿入された分だけ値が引かれる) 時間がカウントされる。


### タイムゾーン
このク�ックのタイムゾーンは、UNIX時間を使用していることからUTCである。このク�ックの時間点をそのまま出力すると、�ーカルタイムゾーンの日時ではなくUTCタイムゾーンの日時が出力されることに注意。

[`utc_clock`](utc_clock.md)はうるう秒をカウントするが、このク�ックはカウントしない違いがある。


### 分解能
このク�ックの分解能は未規定。実装では以下のようになっている：
