...
- C++11 以降では、処理系が IEC 60559 に準拠している場合（[`std::numeric_limits`](../limits/numeric_limits.md)`<T>::`[`is_iec559`](../limits/numeric_limits/is_iec559.md)`() != false`）、以下の規定が追加される。（複号同順）
	...
	- `y = ±0` で `x < 0` または `x = -0` の場合、`±π` が返る。
	- `y = ±0` で `x > 0` または `x = +0` の場合、`±0` が返る。
	- `y > 0` で `x = ±0` の場合、`+π/2` が返る。
	- `y < 0` で `x = ±0` の場合、`-π/2` が返る。
	- `0 < z < +∞` とすると、`y = ±z` で `x = -∞` の場合、`±π` が返る。
	- `0 < z < +∞` とすると、`y = ±z` で `x = +∞` の場合、`±0` が返る。
	- `y = ±∞` で `x` が有限の値の場合、`±π/2` が返る。
	- `y = ±∞` で `x = -∞` の場合、`±3π/4` が返る。
	- `y = ±∞` で `x = +∞` の場合、`±π/4` が返る。

	特に、`y` と `x` の両方がゼロの場合に定義域エラー（[`FE_INVALID`](../cfenv/fe_invalid.md) 浮動小数点例外発生）となったり、`y` が非ゼロで `x` がゼロの場合に極エラー（[`FE_DIVBYZERO`](../cfenv/fe_divbyzero.md) 浮動小数点例外発生）となったりは**しない**事に注意。  
