...
## 戻り値
引数 `x` で指定された値の数値分類を表すマク�の整数値を返す。

| 引数 `x` で指定された値の数値分類 | 戻り値                            |
|-----------------------------------|-----------------------------------|
| �、あるいは、負の無限大の場合    | [`FP_INFINITE`](fp_infinite.md)   |
| NaN（Not a Number、非数）の場合   | [`FP_NAN`](fp_nan.md)             |
| �規化数の場合                    | [`FP_NORMAL`](fp_normal.md)       |
| 非�規化数の場合                  | [`FP_SUBNORMAL`](fp_subnormal.md) |
| �、あるいは、負のゼ�の場合      | [`FP_ZERO`](fp_zero.md)           |

なお、上記のいずれにも属さない処理系定義の数値分類がある場合、その数値分類に対応するマク�の整数値が返る。


## 備考
本関数は、C 言語の `fpclassify` マク�と�価の機能を持つ。
