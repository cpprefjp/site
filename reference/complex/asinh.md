...
- 本関数は、C99 の規格にある `casinh`（より正確には `complex.h` ヘッダの `casinh`、`casinhf`、`casinhl` の 3 つ。それぞれ C++ の `asinh<double>`、`asinh<float>`、`asinh<long double>` に相当）と同等である。
	...
	- 有限で正の符号を持つ（`+0` を含む）実部 `x` について、`asinh(complex(x, +∞))` は `complex(+∞, π/2)` を返す。
	- 有限の実部 `x` について、`asinh(complex(x, NaN))` は `complex(NaN, NaN)` を返すとともに、無効演算の浮動小数点例外（`FE_INVALID`）を引き起こす可能性がある。
	- 有限で正の符号を持つ（`+0` を含む）虚部 `y` について、`asinh(complex(+∞, y))` は `complex(+∞, +0)` を返す。
	- `asinh(complex(+∞, +∞))` は `complex(+∞, π/4)` を返す。
	- `asinh(complex(+∞, NaN))` は `complex(+∞, NaN)` を返す。
	- `asinh(complex(NaN, +0))` は `complex(NaN, +0)` を返す。
	- 有限で非ゼロな虚部 `y` について、`asinh(complex(NaN, y))` は `complex(NaN, NaN)` を返すとともに、無効演算の浮動小数点例外（`FE_INVALID`）を引き起こす可能性がある。
...
- 逆双曲線正弦の算出については、一部の算術型、および、[`valarray`](/reference/valarray.md) クラステンプレートに対しても、他のヘッダで定義されている。

	| 引数の型                                | 関数                                     | ヘッダ                               | 備考       |
	|-----------------------------------------|------------------------------------------|--------------------------------------|------------|
	| `float`                                 | [`asinh`](/reference/cmath/asinh.md)     | [`cmath`](/reference/cmath.md)       |            |
	| `double`                                | [`asinh`](/reference/cmath/asinh.md)     | [`cmath`](/reference/cmath.md)       |            |
	| `long double`                           | [`asinh`](/reference/cmath/asinh.md)     | [`cmath`](/reference/cmath.md)       |            |
	| 任意の整数型                            | [`asinh`](/reference/cmath/asinh.md)     | [`cmath`](/reference/cmath.md)       | C++11 から |
	| [`valarray<T>`](/reference/valarray.md) | [`asinh`](/reference/valarray/asinh.md)  | [`valarray`](/reference/valarray.md) |            |

- 規格書では arc hyperbolic sine となっているが、逆双曲線関数で求めるのは「弧（arc）」ではなく「面積（area）」であるため、（広く誤用されてはいるものの）適切な表現ではない。（[逆双曲線関数 - Wikipedia](http://ja.wikipedia.org/wiki/逆双曲線関数)）
