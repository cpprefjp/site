...
- 本関数は、C99 の規格にある `cacos`（より�確には `complex.h` ヘッダの `cacos`、`cacosf`、`cacosl` の 3 つ。それぞれ C++ の `acos<double>`、`acos<float>`、`acos<long double>` に相当）と�価である。  
	...
	- 有限で非ゼ�の `x` について、`acos(x, NaN)` は `complex(NaN, NaN)` を返すとともに、無効演算の浮動小数点例外（`FE_INVALID`）を引き起こす可能性がある。
	- 有限で�の符号を持つ（`+0` を含む）`y` について、`acos(-∞, y)` は `complex(π, -∞)` を返す。
	- 有限で�の符号を持つ（`+0` を含む）`y` について、`acos(+∞, y)` は `complex(+0, -∞)` を返す。
