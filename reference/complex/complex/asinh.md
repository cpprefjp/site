...
## 概要
複素数値の逆双曲線�弦（エリアハイパボリックサイン：area hyperbolic sine（備考参照））を得る。


## 戻り値
引数 `x` の逆双曲線�弦。本関数の値域は、虚軸方向は全域、実軸方向は `[-π/2, +π/2]` の区間である。


## 備考
- 本関数は実軸の区間 `[-1, +1]` の外側を分岐截�とする。
- 本関数は、C99 の規格にある `casinh`（より�確には `complex.h` ヘッダの `casinh`、`casinhf`、`casinhl` の 3 つ。それぞれ C++ の `asinh<double>`、`asinh<float>`、`asinh<long double>` に相当）と�価である。  
	...
	- 有限で�の符号を持つ（`+0` を含む）`x` について、`asinh(complex(x, +∞))` は `complex(+∞, π/2)` を返す。
	- 有限の `x` について、`asinh(complex(x, NaN))` は `complex(NaN, NaN)` を返すとともに、無効演算の浮動小数点例外（`FE_INVALID`）を引き起こす可能性がある。
	- 有限で�の符号を持つ（`+0` を含む）`y` について、`asinh(complex(+∞, y))` は `complex(+∞, +0)` を返す。
	- `asinh(complex(+∞, +∞))` は `complex(+∞, π/4)` を返す。
	- `asinh(complex(+∞, NaN))` は `complex(+∞, NaN)` を返す。
	- `asinh(complex(NaN, +0))` は `complex(NaN, +0)` を返す。
	- 有限で非ゼ�の `y` について、`asinh(complex(NaN, y))` は `complex(NaN, NaN)` を返すとともに、無効演算の浮動小数点例外（`FE_INVALID`）を引き起こす可能性がある。
...
- 逆双曲線�弦の算出については、一部の算術型に対しても、他のヘッダで定義されている。
	...
	なお、残念ながら [`valarray`](/reference/valarray.md) クラステンプレートには逆双曲線�弦の算出は定義されていない。
...
| [`asinh`](/reference/cmath/asinh.md) | 実数の逆双曲線�弦を求める。              |
