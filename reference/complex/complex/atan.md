...
## 概要
複素数値の逆�接（アークタンジェント：arc tangent）を得る。


## 戻り値
引数 `x` の逆�接。本関数の値域は、虚軸方向は全域で、実軸方向は `[-π/2, +π/2]` の区間である。


## 備考
- 本関数は虚軸の区間 `[-`*i*`, +`*i*`]` の外側を分岐截�とする（*i* は虚数単位）。
- 本関数は、C99 の規格にある `catan`（より�確には `complex.h` ヘッダの `catan`、`catanf`、`catanl` の 3 つ。それぞれ C++ の `atan<double>`、`atan<float>`、`atan<long double>` に相当）と�価である。  
...
- 逆�接の算出については、一部の算術型、および、[`valarray`](/reference/valarray.md) クラステンプレートに対しても、他のヘッダで定義されている。
...
| [`atan`](/reference/cmath/atan.md) | 実数の逆�接を求める。                    |
