...
## 概要
複素数値の逆双曲線�接（エリアハイパボリック：area hyperbolic tangent（備考参照））を得る。


## 戻り値
引数 `x` の双曲線逆�接。本関数の値域は、実軸方向は全域、虚軸方向は `[-`*i* `π/2, +`*i* `π/2]` の区間である（*i* は虚数単位）。


## 備考
- 本関数は実軸の区間 `[-1, +1]` の外側を分岐截�とする。
- 本関数は、C99 の規格にある `catanh`（より�確には `complex.h` ヘッダの `catanh`、`catanhf`、`catanhl` の 3 つ。それぞれ C++ の `atanh<double>`、`atanh<float>`、`atanh<long double>` に相当）と�価である。  
	...
	- `atanh(complex(+1, +0))` は `complex(+∞, +0)` を返すとともに、ゼ�除算の浮動小数点例外（`FE_DIVBYZERO`）を引き起こす。
	- 有限で�の符号を持つ（`+0` を含む）`x` について、`atanh(complex(x, +∞))` は `complex(+0, π/2)` を返す。
	- 有限で非ゼ�の値を持つ `x` について、`atanh(complex(x, NaN))` は `complex(NaN, NaN)` を返すとともに、無効演算の浮動小数点例外（`FE_INVALID`）を引き起こす可能性がある。
	- 有限で�の符号を持つ（`+0` を含む）`y` について、`atanh(complex(+∞, y))` は `complex(+0, π/2)`を返す。
...
- 逆双曲線�接の算出については、一部の算術型に対しても、他のヘッダで定義されている。
	...
	なお、残念ながら [`valarray`](/reference/valarray.md) クラステンプレートには逆双曲線�接の算出は定義されていない。
...
| [`atanh`](/reference/cmath/atanh.md) | 実数の逆双曲線�接を求める。              |
