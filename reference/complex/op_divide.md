...
##備考
`lhs` と `rhs` に現れる型 `T` は（残念ながら）同じ型でなければならない。（`complex<double>` と `complex<float>` の除算などを行うことはできない。）  
特に、`rhs` の型が `const T&` の場合でも、[`operator/=`](complex/op_divide_assign.md) の場合と異なり、`complex<double>` と `float` の除算などを行うことはできないので注意。
