...
以下のマクロは、各整数型の定数値を表す関数マクロである。例：) <code style='font-weight:normal'>int8_t x = INT8_C(1);</code>これらのマクロは、`__STDC_CONSTANT_MACROS`マクロが定義されている場合のみ定義される。符号あり整数型用の定数値マクロ
