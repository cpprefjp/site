...
| [`CLOCKS_PER_SEC`](ctime/clocks_per_sec.md) | `clock()`関数によって返される1秒あたりの数値 | |
...
| [`__STDC_VERSION_TIME_H__`](ctime/stdc_version_time_h.md) | `<ctime>`が提供するC標準ライブラリ機能のバージョン (`202311L`) | C++26 |
...
| [`clock_t`](ctime/clock_t.md) | `clock()`関数が返すプロセッサ時間を表す型 | |
...
| 名前 | 説明 | 対応バージョン |
|------|------|----------------|
| [`clock`](ctime/clock.md)        | プログラム実行開始からの経過時間を取得する | |
| [`difftime`](ctime/difftime.md)     | 2つの時間の差を計算する | |
| [`mktime`](ctime/mktime.md)       | カレンダー時間から経過秒を生成する | |
| [`timegm`](ctime/timegm.md)       | UTCのカレンダー時間から経過秒を生成する | C++26 |
| [`gmtime`](ctime/gmtime.md)       | 経過秒からカレンダー時間を生成する | |
| [`gmtime_r`](ctime/gmtime_r.md)     | 経過秒からカレンダー時間を生成する (利用者が用意したバッファに格納する) | C++26 |
| [`localtime`](ctime/localtime.md)    | 経過秒からローカル時間のカレンダー時間を生成する | |
| [`localtime_r`](ctime/localtime_r.md)  | 経過秒からローカル時間のカレンダー時間を生成する (利用者が用意したバッファに格納する) | C++26 |
| [`time`](ctime/time.md)         | 現在時間までの経過秒を取得する | |
...
| [`strftime`](ctime/strftime.md)     | 指定したフォーマットでカレンダー時間を文字列化する | |
