...
| `clock_t` | `clock()`関数が返すプ�セッサ時間を表す型 | |
...
| `asctime`      | カレンダー時間を文�列化する | |
| `ctime`        | 経過秒を日時として文�列化する | |
| `localtime`    | �ーカル時間を取得する | |
| `strftime`     | 指定したフォーマットでカレンダー時間を文�列化する | |
