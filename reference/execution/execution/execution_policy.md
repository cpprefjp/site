...
いずれの実行ポリシーでも、実行�に例外が送出され、それが捕捉されなかった場合は、[`std::terminate()`](/reference/exception/terminate.md)関数が呼び出され、プ�グラムが異常終了する。


### sequenced_policy / seq
この逐次実行の実行ポリシーは、実行ポリシーを指定せずにアルゴリズムを実行することと�価である。
...
この実行ポリシーでも`parallel_policy`と同様に、副作用をともなう処理でデータ競合が発生する可能性がある。ただし、マルチスレッド化だけでなくベクトル化も組み合わさるために、ミューテックスによる排他処理をした場合には、複数回�ックが取得されてデッド�ックが発生する可能性がある。そのため、この実行ポリシーではLock-freeアトミック操作によってデータ競合を回避する必要がある。
...
この実行ポリシーで、ミューテックスによる排他処理を行った場合、デッド�ックが発生する可能性がある。この実行ポリシーはシングルスレッドで並行に複数データを処理するため、同じスレッド内で�ック取得操作を複数回呼び出すとデッド�ックする。
...
## 備考
- これらの実行ポリシーで並列アルゴリズムを実行する際、要素アクセスの関数が例外を送出した場合は、[`std::terminate()`](/reference/exception/terminate.md)関数が呼び出され、プ�グラムが異常終了する


## 計算量
実行ポリシーをとらないアルゴリズムは「最大N回だけ関数`f()`を呼び出す」や「�確にN回だけ関数`f()`を呼び出す」のように計算量を規定する。実行ポリシーはそれを緩和し、ビッグオー記法を使用して「O(N)計算量の回数だけ関数`f()`を呼び出す」のように表記する。
