...
この関数は、オペレーティング・システムのファイルシステムを介して、パスの�規化を行う。ファイルシステムを使用せず、文�列処理として�規化したい場合は、[`path::lexically_normal()`](path/lexically_normal.md)関数を使用する。

この関数は、�規化対象となるパスが指すファイルが�在していなければならない。�在しないファイルのパスを�規化したい場合は、[`weakly_canonical()`](weakly_canonical.md)関数を使用する。


## 効果
パス`p`に含まれる`"."` (ドットx1)、`".."` (ドットx2) を解決し、絶対パスに変換する。

パス`p`のファイル・ディレクトリが�在しない場合はエラーとなる。(2)ではエラーとなった場合、`ec`にエラー情報が書き込まれる。
