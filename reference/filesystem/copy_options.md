...
それぞれの列挙�の値は未規定だが、異なる値を持つ。`none`は空のビットマスク値を表す。

以下に、オプションのグループごとに列挙�の意味を記載する：

### `copy_file()`関数での、�在するファイルに対する動作

| 列挙� | 説明 |
|--------|------|
| `none`               | すでにファイルが�在している場合はエラー (デフォルト) |
| `skip_existing`      | すでに�在しているファイルは、上書きせず、エラー報告しない |
| `overwrite_existing` | すでに�在しているファイルを上書きする |
| `update_existing`    | すでに�在しているファイルがコピー元よりも古ければ上書きする |
