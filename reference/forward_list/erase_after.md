...
削除された要素の次の要素を指すイテレータを返す。そのような要素が存在しない場合は、[`end()`](/reference/forward_list/end.md)を返す。
...
```
* erase_after(std::next(ls.before_begin(), 3));[color ff0000]
