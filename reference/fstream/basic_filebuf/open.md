...
まず`mode & ~ios_base​::​ate`の結果からファイルの開くモードが決定される。`fopen`のモード文�列との対応は以下の通り。

| `binary` | `in` | `out` | `trunc` | `ate` | 対応する`fopen`のモード文�列 |
|----------|------|-------|---------|-------|--------------------|
|          |      | ○     |         |       | `"w"`              |
|          |      | ○     |         | ○     | `"a"`              |
|          |      |       |         | ○     | `"a"`              |
...
そしてあたかも`fopen`がこのモード文�列を第二引数に指定して呼び出されたかのように振る舞う。
