...
ボイヤー・ムーア・ホースプール法は、ボイヤー・ムーア法を簡略化したアルゴリズムである。ボイヤー・ムーア法よりも、初期化とループごとの処理で少しだけオーバーヘッドが少ないが、最良ケースでのビッグオー記法での計算量は、どちらも同じとなる。最悪計算量はボイヤー・ムーア法よりも高くなるため、入力データによって性能が変わる。
...
| 名前 | 説明 | 対応バージョン |
|------|------|----------------|
| [`(constructor)`](boyer_moore_horspool_searcher/op_constructor.md.nolink) | コンストラクタ | C++17 |
| [`operator()`](boyer_moore_horspool_searcher/op_call.md.nolink) | 検索を実行する | C++17 |
