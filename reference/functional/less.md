...
## 備考
- `less<void>`の`operator()`が組み込みのポインタ比較演算子を呼び出す場合、その比較は厳密な全順序を与える。この順序は、`less`/`greater`/`less_equal`/`greater_equal`の各特殊化の間で一貫しており、かつ組み込みのポインタ比較演算子が定義される場合はその結果とも一致する。
...
- [LWG Issue 2450. `(greater|less|greater_equal|less_equal)<void>` do not yield a total order for pointers](https://cplusplus.github.io/LWG/issue2450)
    - C++17で、`less<void>`がポインタを比較する場合に全順序を与えることが規定された（非`void`版と同様）
