...
## 要件
各`placeholder`の型は、デフォルト構築とコピー構築が可能であり、デフォルト・コピー・ムーブコンストラクタはいかなる例外も投げない。
また、それらの型がコピー代入可能であるかは処理系定義であるが、コピー代入可能な場合はその代入演算子も同様にいかなる例外も投げない。

C++20以降、それらのコンストラクタ及び（定義されている場合）代入演算子は`constexpr`関数である。
