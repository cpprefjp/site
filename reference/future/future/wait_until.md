...
<b>絶対時間でタイムアウトを指定して、処理が完了するまで待機する</b>


##効果

共有状態が遅延状態([`future_status::deferred`](/reference/future/future_status.md))の場合、この関数は何もしない。そうでない場合、`abs_time`で指定された絶対時間の期限まで、共有状態が準備状態([`future_status::ready`](/reference/future/future_status.md))になるのを待機する。
...
- [`future_status::timeout`](/reference/future/future_status.md) ： `abs_time`で指定された絶対時間内に準備完了にならず、タイムアウトになった
