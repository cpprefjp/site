...
`packaged_task`は、「別スレッドでの処理完了を待ち、その処理結果を取得する」といった非同期処理を実現するためのクラスであり、[`future`](/reference/future/future.md)クラスと組み合わせて使用する。`packaged_taskに登録した非同期実行する関数の戻り値を`[`future`](/reference/future/future.md)が読み取る。`packaged_task`と[`future`](/reference/future/future.md)は内部的に同一の共有状態を参照する。テンプレートパラメータ：
