...
## 概要
範囲の先�の前を指す逆イテレータを取得する。

- (1) : コンテナの`rend()`メンバ関数で、範囲の先�の前を指す、逆イテレータを返す。
- (2) : コンテナの`rend()`メンバ関数で、範囲の先�の前を指す、�み取り専用逆イテレータを返す。
- (3) : 組み込み配列の先�の前を指す、逆イテレータを返す。
- (4) : `initializer_list`オブジェクトの先�の前を指す、�み取り専用逆イテレータを返す。
