...
## 概要
`locale`を引数で指定できる`isprint()`関数。
文字`c`が印字可能文字であるかどうかを、ロケールに基いて判定する。


## 戻り値
`std::`[`use_facet`](use_facet.md)`<std::`[`ctype`](ctype.md)`<charT>>(loc).is(std::`[`ctype_base`](ctype_base.md)`::print, c)`を返す。


## 備考
`locale`を引数に取らない`isprint()`関数は、[`<cctype>`](/reference/cctype.md)ヘッダに存在する。
...
## 関連項目
- [`ctype`](ctype.md)
- [`isprint - <cctype>`](/reference/cctype/isprint.md)
