...
引数に `first` 、`last` をとるバージョンはコンテナの [`size()`](/reference/map/map/size.md) についての対数時間、プラス `first` と `last` の間の距離に対する線形時間。 
引数に `x` をとるバージョンはコンテナの [`size()`](/reference/map/map/size.md) について対数時間。
