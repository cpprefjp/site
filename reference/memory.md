...
## フリースタンディング
このヘッダのほとんどの機能は、フリースタンディング処理系でも使用できる。ただし、実行ポリシーを取る並列アルゴリズムのオーバーロードは、フリースタンディング処理系では削除される。
...
| [`start_lifetime`](memory/start_lifetime.md) | 参照先のオブジェクトの生存期間を明示的に開始する(function template) | C++26 |
