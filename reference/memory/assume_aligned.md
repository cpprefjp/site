...
より具体的には連続したメモリー領域へのアクセスにおけるキャッシュ最適化や SIMD 命令を使用した処理速度やフットプリントの最適化の可能性に対し、メモリー領域のアライメントをプログラマーによって明示的にコンパイラーへ与える事で、より良好な最適化をコンパイラーが選択し得る可能性を与える。

（但し、本機能の使用により実際にどのような最適化にどの程度の効果が得られるかは処理系や最適化オプションにより異なる。プログラマーが期待し得た最適化を必ずしもコンパイラーが行えるようになるとは限らない。）
...
この機能によって引数または戻り値として得られるメモリー領域が変化する事はない。アライメント不定のメモリー領域を本機能へ渡してもアライメントされたメモリー領域を得られるわけではない。アライメントされたメモリー領域を得たい場合には、先ず `alignas` を参照すると良い。
