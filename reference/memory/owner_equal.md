...
## 概要
`owner_equal`は、スマートポインタを所有権ベースで等値比較するための関数オブジェクトである。

スマートポインタを、[`unordered_set`](/reference/unordered_set/unordered_set.md)や[`unordered_map`](/reference/unordered_map/unordered_map.md)のキーにする際、値ベースではなく所有権ベースに比較することを指定するために使用する。

[`owner_hash`](owner_hash.md)と組み合わせて使用する。
...
| 名前 | 説明 | 対応バージョン |
|----------------------------------------|------|-------|
| [`operator()`](owner_equal/op_call.md) | 関数呼び出し演算子 | C++26 |
...
```
* std::owner_equal[color ff0000]
* std::owner_hash[link owner_hash.md]
...
## 関連項目
- [`std::owner_hash`](owner_hash.md)
- [`std::owner_less`](owner_less.md)
- [`std::shared_ptr::owner_equal()`](shared_ptr/owner_equal.md)
- [`std::weak_ptr::owner_equal()`](weak_ptr/owner_equal.md)
