...
## 概要
例外を送出せずに呼び出し可能かを判定する。[`std::is_nothrow_invocable`](/reference/type_traits/is_nothrow_invocable.md)に対応する。


## 戻り値
`type`が`type_args`で例外を送出せずに呼び出し可能な場合に`true`を返す。
...
## 関連項目
- [`std::is_nothrow_invocable`](/reference/type_traits/is_nothrow_invocable.md)
