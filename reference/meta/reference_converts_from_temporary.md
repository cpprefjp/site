...
## 概要
参照への変換が一時オブジェクトを経由するかを判定する。[`std::reference_converts_from_temporary`](/reference/type_traits/reference_converts_from_temporary.md)に対応する。


## 戻り値
`type_dst`の参照が`type_src`からの暗黙変換で一時オブジェクトに束縛される場合に`true`を返す。
