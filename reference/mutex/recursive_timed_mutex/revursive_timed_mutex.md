...
<li>`recursive_timed_mutex()`デフォルトコンストラクタ。`recursive_timed_mutex`オブジェクトの初期化を行う。
</li><li>`recursive_timed_mutex(const recursive_timed_mutex&) = delete`コピーコンストラクタ。コピー不可。
