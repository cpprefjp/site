...
- (5) : 単純な記憶域の解放。例外を送出しない�定で記憶域を確保されたオブジェクトに対する、配列オブジェクトの記憶域の解放
- (6) : 単純な記憶域の解放。例外を送出しない�定で記憶域を確保されたオブジェクトに対する、デフォルトより大きいアライメント要求の、配列オブジェクトの記憶域の解放
- (7) : 配置newで確保された配列オブジェクトの記憶域の解放


## 効果
[`new[]`](op_new[].md)演算�によって動的に確保した記憶域を解放する。

`nothrow`版の`delete[]`演算�は、`nothrow`版の[`new[]`](op_new[].md)演算�によって生成されたオブジェクトのコンストラクタが例外を送出した場合に呼び出される。


## 備考
- `alignment` 引数を取らない `delete[]` 演算�に渡される `ptr` は、 `alignment` 引数を取らない `new[]` 演算�によって確保されたものでなければならない。
- `alignment` 引数を取る `delete[]` 演算�に渡される `ptr` は、同じ `alignment` の値を取った `alignment` 版 `new[]` 演算�によって確保されたものでなければならない。
