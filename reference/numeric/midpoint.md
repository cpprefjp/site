...
この関数は、単純に計算する場合は`(a + b) / 2`となるが、オーバーフ�ーが起こりにくいよう計算式に工夫が入る。例として、整数の場合は`a + (b - a) / 2`、浮動小数点数の場合は`a/2 + b/2`のように計算できる (実際は負数や非�規化数などをさらに考慮することになる)。
...
- (1) : `a`と`b`を合計した値の1/2の数値を返す
    ...
    - 型`T`が浮動小数点数型の場合、最大一回の[不�確 (inexact) 操作](/reference/cfenv/fe_inexact.md)が発生し、オーバーフ�ーは起こらない
- (2) : `a`と`b`が同じ配列`x`の`x[i]`と`x[j]`を指しているとして、`x[i + (j - i) / 2]`を指すポインタを返す。除算の結果はゼ�方向に丸められる
...
## 備考
- 浮動小数点数の文脈では、この関数の動作は平均値 (mean value) という名称を使用するが、それ以外の文脈では�点が使用される。汎用的な用途とするため、この関数の名前として�点 (midpoint) を採用している
