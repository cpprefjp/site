...
先に挙げた乱数生成器に対し一般的なパラメータを定義し、使いやすいように用意された乱数生成器の型。
...
- シード生成には、[`random_device`](./random/random_device.md)クラスを使用する。
    - 古典的な方法として、シード生成には`std::time(NULL)`の呼び出しによる、現在時間(エポックからの経過秒)が使われていた。この方法では、アプリケーションの起動時間から、シードが推測されてしまう可能性がある。シードが推測されれば、その後の乱数のシーケンスが推測されてしまい、セキュリティホールになろ恐れがある。
    - [`random_device`](./random/random_device.md)クラスをシード生成に使用することで、シードの予測を困難にできる。
- 擬似乱数エンジンは、基本的には32ビット版メルセンヌ・ツイスターの実装である[`mt19937`](./random/mt19937.md)、もしくはその64ビット版の実装である[`mt19937_64`](./random/mt19937_64.md)のどちらかを使用することを推奨する。
    ...
    - [`mt19937`](./random/mt19937.md)は、状態の大きさが`sizeof(std::uint_fast32_t) * (624 + 1)`だけあり、少々サイズが大きいが、それを受け入れられる状況であれば、乱雑度や周期の長さが問題になることは少ない。
- 特定の範囲の整数や浮動小数点数が必要な場合は、[`uniform_int_distribution`](./random/uniform_int_distribution.md)や[`uniform_real_distribution`](./random/uniform_real_distribution)のような、専門特化した分布クラスを使用する。
    - 古典的な方法として、特定の範囲の整数を生成するために、剰余演算が使われていた。たとえば`[0, 6)`の範囲のランダムな整数が必要な場合には、`int x = random_value % 6;`のようにしていた。この方法にはmodulo biasと呼ばれる値の偏り問題が発生する。
