...
コンテナとは言っても [`regex_match`](regex_match.md.nolink) や [`regex_search`](regex_search.md.nolink) のマッチ結果を格納することを目的としているため、
...
[`regex_match`](regex_match.md.nolink) や [`regex_search`](regex_search.md.nolink) に引数として渡されると、マッチが成功したか否かにかかわらずその結果を格納する（[`ready`](match_results/ready.md)`() == true`）。  
