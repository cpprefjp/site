...
##備考
- [`regex_match`](../regex_match.md.nolink)、および、[`regex_search`](../regex_search.md.nolink) の引数に渡した [`match_results`](../match_results.md) オブジェクトは、マッチが成功すると `empty() != true` となる。  
...
- `empty() == true` の場合、[`match_results`](../match_results.md) のメンバ関数には結果が未規定であるものがあるので、注意が必要である（[`regex_match`](../regex_match.md.nolink)、および、[`regex_search`](../regex_search.md.nolink) を参照）。
