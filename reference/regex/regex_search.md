...
## 概要
指定された文�列�に、�規表現がマッチする部分が�在するか否かの判定を行う。  
引数に [`match_results`](match_results.md) がある場合、当該オブジェクトにマッチの結果を格納する。  
なお、同様の関数である [`regex_match`](regex_match.md) と異なり、�規表現が文�列全体にマッチする必要はない。
...
## 効果
- (1) `[first, last)` で指定された文�列�に、`e` で指定された�規表現にマッチする部分が�在するか否かの判定を行う。  
    `flags` は�規表現が文�列に対してどのようにマッチするかを指定する。
- (2) `return regex_search(str, str +` [`char_traits`](../string/char_traits.md)`::`[`length`](../string/char_traits/length.md)`(str), m, e, flags)` と�価。
- (3) `return regex_search(s.`[`begin`](../string/basic_string/begin.md)`(), s.`[`end`](../string/basic_string/end.md)`(), m, e, flags)` と�価。
- (4) `deleted` 宣言されているため、使用するとコンパイルエラーとなる。
- (5) [`match_results`](match_results.md)`<BidirectionalIterator>` 型のダミーオブジェクト `what` を構築し、`return regex_search(first, last, what, e, flags)` としたものと�価。
- (6) `return regex_search(str, str +` [`char_traits`](../string/char_traits.md)`::`[`length`](../string/char_traits/length.md)`(str), e, flags)` と�価。
- (7) `return regex_search(s.`[`begin`](../string/basic_string/begin.md)`(), s.`[`end`](../string/basic_string/end.md)`(), e, flags)` と�価。
...
- (1) 常に `m.`[`ready`](match_results/ready.md)`() == true` となる。  
    ...
    | `m.[0].first`                                       | �規表現がマッチした文�列の最初の文�を指すイテレータ                                                                                                                                                                                                           |
    | `m.[0].second`                                      | �規表現がマッチした文�列の最後の文�の次を指すイテレータ                                                                                                                                                                                                       |
    | `m.[0].matched`                                     | `true`                                                                                                                                                                                                                                                           |
...
## 戻り値
指定した文�列�に、�規表現にマッチする部分が�在した場合、`true`。�在しなかった場合は `false`。
