...
`set`は�ーの重複を許可しないが、`multiset`は�ーの重複を許可する。  
...
- ユニークな要素の�ー：互いに�しい二つの�ーを持つ要素が `set` に格納されることは無い。複数の�しい�ーを許す同様の連想コンテナは `multiset` である。
...
| 名前 | 説明 | 対応バージョン |
|---------------------------------|--------------------------------------------|-------|
| [`set`](set/set.md)           | �ーの重複を許可しない集合(class template) |  |
| [`multiset`](set/multiset.md) | �ーの重複を許可する集合(class template)   |  |
