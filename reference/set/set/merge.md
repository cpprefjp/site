...
`source` の要素の�ーと�価な�ーの要素がある場合、その要素は `source` から抽出されない。  
...
## 備考
`source` の転送された要素へのポインタおよび参照は、それらと同じ要素を参照するが、`*this` のメンバとして参照する。また、転送された要素を参照する反復�は、引き続きその要素を参照するが、転送後は `source` ではなく `*this` への反復�として動作する。


## 例外 
比較オブジェクトが例外をス�ーしない限り、例外をス�ーしない。
