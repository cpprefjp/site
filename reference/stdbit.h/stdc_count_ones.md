...
```cpp
template<class T>
unsigned int stdc_count_ones(T value);
...
int main() {
  assert(stdc_count_ones(static_cast<std::uint8_t>(0b1111'0000)) == 4);
  assert(stdc_count_ones(static_cast<std::uint8_t>(0b0000'0000)) == 0);
  assert(stdc_count_ones(static_cast<std::uint8_t>(0b1111'1111)) == 8);

  // 型別関数
  assert(stdc_count_ones_uc(static_cast<unsigned char>(0b1111'0000)) == 4);
}
```
* stdc_count_ones[color ff0000]
...
## 関連項目
- [`std::popcount`](/reference/bit/popcount.md) - C++標準の対応する機能
