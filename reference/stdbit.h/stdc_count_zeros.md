...
```cpp
template<class T>
unsigned int stdc_count_zeros(T value);
...
int main() {
  assert(stdc_count_zeros(static_cast<std::uint8_t>(0b1111'0000)) == 4);
  assert(stdc_count_zeros(static_cast<std::uint8_t>(0b0000'0000)) == 8);
  assert(stdc_count_zeros(static_cast<std::uint8_t>(0b1111'1111)) == 0);

  // 型別関数
  assert(stdc_count_zeros_uc(static_cast<unsigned char>(0b1111'0000)) == 4);
}
```
* stdc_count_zeros[color ff0000]
