...
```cpp
template<class T>
unsigned int stdc_first_leading_one(T value);
...
## 戻り値
値`value`の最上位ビット (MSB) 側から数えて、最初の1ビットの位置を1始まりで返す。すべてのビットが0の場合は0を返す。
...
int main() {
  // 0b00001111 の最初の1は5番目 (MSB側から)
  assert(stdc_first_leading_one(static_cast<std::uint8_t>(0b0000'1111)) == 5);
  // すべて0なら0を返す
  assert(stdc_first_leading_one(static_cast<std::uint8_t>(0)) == 0);
}
```
* stdc_first_leading_one[color ff0000]
