...
```cpp
template<class T>
unsigned int stdc_first_trailing_zero(T value);
...
## 戻り値
値`value`の最下位ビット (LSB) 側から数えて、最初の0ビットの位置を1始まりで返す。すべてのビットが1の場合は0を返す。
...
int main() {
  // 0b00001111 の最初の0は5番目 (LSB側から)
  assert(stdc_first_trailing_zero(static_cast<std::uint8_t>(0b0000'1111)) == 5);
  // すべて1なら0を返す
  assert(stdc_first_trailing_zero(static_cast<std::uint8_t>(0xFF)) == 0);
}
```
* stdc_first_trailing_zero[color ff0000]
