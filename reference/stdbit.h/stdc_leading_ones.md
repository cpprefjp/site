...
```cpp
template<class T>
unsigned int stdc_leading_ones(T value);
...
## 戻り値
値`value`の、最上位ビット (MSB) から開始して連続した1ビットの数を返す。
...
int main() {
  assert(stdc_leading_ones(static_cast<std::uint8_t>(0b1110'0000)) == 3);
  assert(stdc_leading_ones(static_cast<std::uint8_t>(0xFF)) == 8);

  // 型別関数
  assert(stdc_leading_ones_uc(static_cast<unsigned char>(0xFF)) == 8);
}
```
* stdc_leading_ones[color ff0000]
...
## 関連項目
- [`std::countl_one`](/reference/bit/countl_one.md) - C++標準の対応する機能
