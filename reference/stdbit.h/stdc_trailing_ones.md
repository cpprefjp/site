...
```cpp
template<class T>
unsigned int stdc_trailing_ones(T value);
...
## 戻り値
値`value`の、最下位ビット (LSB) から開始して連続した1ビットの数を返す。
...
int main() {
  assert(stdc_trailing_ones(static_cast<std::uint8_t>(0b0000'0111)) == 3);
  assert(stdc_trailing_ones(static_cast<std::uint8_t>(0xFF)) == 8);

  // 型別関数
  assert(stdc_trailing_ones_uc(static_cast<unsigned char>(0xFF)) == 8);
}
```
* stdc_trailing_ones[color ff0000]
...
## 関連項目
- [`std::countr_one`](/reference/bit/countr_one.md) - C++標準の対応する機能
