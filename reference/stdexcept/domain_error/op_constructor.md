...
```cpp
explicit domain_error(const string& what_arg);           // (1) C++98
constexpr explicit domain_error(const string& what_arg); // (1) C++26

explicit domain_error(const char* what_arg);             // (2) C++11
constexpr explicit domain_error(const char* what_arg);   // (2) C++26

domain_error(const domain_error&) noexcept;                     // (3) C++98
constexpr domain_error(const domain_error&) noexcept;           // (3) C++26
```
* string[link /reference/string/basic_string.md]

## 概要
`domain_error`オブジェクトを構築する。
...
int main()
{
  std::domain_error e1{"error message"};              // (2) ヌル終端文字列から構築
  std::domain_error e2{std::string{"error message"}}; // (1) std::stringから構築

  std::cout << e1.what() << std::endl;
  std::cout << e2.what() << std::endl;
}
```
* std::domain_error[color ff0000]
