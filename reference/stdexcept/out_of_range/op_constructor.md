...
constexpr explicit out_of_range(const string& what_arg); // (1) C++26

explicit out_of_range(const char* what_arg);             // (2) C++11
constexpr explicit out_of_range(const char* what_arg);   // (2) C++26

out_of_range(const out_of_range&) noexcept;                     // (3) C++98
constexpr out_of_range(const out_of_range&) noexcept;           // (3) C++26
```
* string[link /reference/string/basic_string.md]

## 概要
`out_of_range`オブジェクトを構築する。
...
int main()
{
  std::out_of_range e1{"error message"};              // (2) ヌル終端文字列から構築
  std::out_of_range e2{std::string{"error message"}}; // (1) std::stringから構築

  std::cout << e1.what() << std::endl;
  std::cout << e2.what() << std::endl;
}
```
* std::out_of_range[color ff0000]
