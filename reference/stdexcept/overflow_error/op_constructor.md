...
```cpp
explicit overflow_error(const string& what_arg);           // (1) C++98
constexpr explicit overflow_error(const string& what_arg); // (1) C++26

explicit overflow_error(const char* what_arg);             // (2) C++11
constexpr explicit overflow_error(const char* what_arg);   // (2) C++26

overflow_error(const overflow_error&) noexcept;                     // (3) C++98
constexpr overflow_error(const overflow_error&) noexcept;           // (3) C++26
```
* string[link /reference/string/basic_string.md]

## 概要
`overflow_error`オブジェクトを構築する。
...
int main()
{
  std::overflow_error e1{"error message"};              // (2) ヌル終端文字列から構築
  std::overflow_error e2{std::string{"error message"}}; // (1) std::stringから構築
