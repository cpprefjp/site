...
```cpp
runtime_error& operator=(const runtime_error&) noexcept;           // (1) C++98
constexpr runtime_error& operator=(const runtime_error&) noexcept; // (1) C++26
```

## 概要
`runtime_error`オブジェクトを代入する。
