...
* std[meta namespace]
* runtime_error[meta class]
* function[meta id-type]
* cpp98[meta cpp]

```cpp
explicit runtime_error(const string& what_arg);           // (1) C++98
constexpr explicit runtime_error(const string& what_arg); // (1) C++26

explicit runtime_error(const char* what_arg);             // (2) C++11
constexpr explicit runtime_error(const char* what_arg);   // (2) C++26

runtime_error(const runtime_error&) noexcept;                     // (3) C++98
constexpr runtime_error(const runtime_error&) noexcept;           // (3) C++26
```
* string[link /reference/string/basic_string.md]

## 概要
`runtime_error`オブジェクトを構築する。
...
int main()
{
  std::runtime_error e1{"error message"};              // (2) ヌル終端文字列から構築
  std::runtime_error e2{std::string{"error message"}}; // (1) std::stringから構築
