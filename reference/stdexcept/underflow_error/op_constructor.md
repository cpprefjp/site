...
```cpp
explicit underflow_error(const string& what_arg);           // (1) C++98
constexpr explicit underflow_error(const string& what_arg); // (1) C++26

explicit underflow_error(const char* what_arg);             // (2) C++11
constexpr explicit underflow_error(const char* what_arg);   // (2) C++26

underflow_error(const underflow_error&) noexcept;                     // (3) C++98
constexpr underflow_error(const underflow_error&) noexcept;           // (3) C++26
```
* string[link /reference/string/basic_string.md]

## 概要
`underflow_error`オブジェクトを構築する。
...
int main()
{
  std::underflow_error e1{"error message"};              // (2) ヌル終端文字列から構築
  std::underflow_error e2{std::string{"error message"}}; // (1) std::stringから構築
