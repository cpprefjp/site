...
テンプレートパラメータとして文字型を受け取るようになっており、使用を容易にするため、以下のようなパラメータ設定済みエイリアスが定義されている。
...
このクラスが表すのは、文字型`charT`の動的配列であり、文字の動的配列ではない。したがって、文字列中にサロゲートペアや結合文字列が含まれていたとしても、それらは`basic_string`クラスにおいて、複数の要素と見なされる。
