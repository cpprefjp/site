...
## 戻り値
C 言語で使用されている文�列表現である、NULL（つまり `charT()`）で終端された文�配列の先�へのポインタ（備考も参照）
...
- 本メンバ関数の呼び出しによる、対象オブジェクトの要素への既�の参照、ポインタ、イテレータの有効性への影響は以下の通りである。
	* C++03 まで：本メンバ関数を呼び出すと、対象オブジェクトの要素への既�の参照、ポインタ、イテレータは無効になる可能性がある。
	* C++11 から：本メンバ関数を呼び出しても、対象オブジェクトの要素への既�の参照、ポインタ、イテレータは無効にはならない。
...
- [`data`](data.md) は、C++03 までは NULL で終端されていない文�配列へのポインタを返していたが、C++11 からは本メンバ関数と全く同じものとなった。
