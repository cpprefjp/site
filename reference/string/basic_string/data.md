...
- (1) : �み取り専用で文�配列の先�へのポインタを取得する
- (2) : 文�配列の先�へのポインタを取得する


## 戻り値
- (1) :
    - C++03 : 文�配列の先�へのポインタを返す。ただし、NULLで終端はされない
    - C++11 : C 言語で使用されている文�列表現である、NULL（つまり `charT()`）で終端された文�配列の先�へのポインタを返す
- (2) :
    - C 言語で使用されている文�列表現である、NULL（つまり `charT()`）で終端された文�配列の先�へのポインタを返す
...
- (1) :
    - C++03まで :
        ...
        - 本メンバ関数を呼び出すと、対象オブジェクトの要素への既�の参照、ポインタ、イテレータは無効になる可能性がある。
        ...
        - 本メンバが返すポインタは、[`size`](size.md)`() != 0` の場合、長さが [`size`](size.md)`()` の `charT` 型の配列を指す。この配列の要素は対象オブジェクトの文�列と�しい。
        - [`size`](size.md)`() == 0` の場合、NULL ポインタ**ではなく**、当該ポインタのコピー、および、当該ポインタに対してゼ�の加算が可能な値である。
    ...
    - C++11から :
        ...
        - 本メンバ関数を呼び出しても、対象オブジェクトの要素への既�の参照、ポインタ、イテレータは無効にはならない。
        - 本メンバ関数で返されたポインタは、対象オブジェクトに対する非constメンバ関数呼び出しにより無効になる可能性がある。
        - 本メンバが返すポインタは、長さが [`size`](size.md)`() + 1` の `charT` 型の配列を指す。この配列は、最初の [`size`](size.md)`()` 要素は対象オブジェクトの文�列と�しく、最後の要素は NULL 文�、すなわち `charT()` である。  
            - 本メンバ関数が返すポインタを `p` とすると、範囲 `[0,` [`size`](size.md)`()]` の全ての `i` について `p + i == &operator[](i)` を満たす。このことから、`*`[`end`](end.md)`() == charT()` を満たす。
- 対象オブジェクト内に NULL 文�があった場合、C 言語の文�列表現では�しく扱うことができないので注意すること。
- (2) :
...
| 名前                  | 説明                           |
|-----------------------|--------------------------------|
| [`c_str`](c_str.md) | C 言語での文�列表現を取得する |
