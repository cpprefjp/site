...
## 概要
文�列`str`を数値として�み取って、`unsigned long`型の値に変換する。
...
- 以下の条件に合致した場合、[`std::out_of_range`](/reference/stdexcept.md)が送出される。
    - `std::strtoul()`関数が[`errno`](/reference/cerrno/errno.md)変数に[`ERANGE`](/reference/cerrno.md)を�定した場合 (C++14)
    - 結果が範囲外の値になった場合
...
int main()
{
  ...
  // 2進法での変換
  {
    ...
    unsigned long xw = std::stoul(L"01001", nullptr, 2); // 先�に0が付いていてもよい
    std::cout << xw << std::endl;
  }
