#代入演算子
```cpp
<pre>// コピーthread& operator=(const thread&) = delete;// ムーブthread& operator=(thread&& x) noexcept;</pre>
```
* コピー[italic]
* ムーブ[italic]
...
```cpp
#include <thread>int main(){  std::thread th1([]{ /*...*/ });  std::thread th2;  assert(th1.joinable() && !th2.joinable());  // th1からth2へムーブ代入  th2 = std::move(th1);  assert(!th1.joinable() && th2.joinable());  th2.join();  return;}
```
