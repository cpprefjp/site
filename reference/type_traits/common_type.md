...
より詳細には、次のように決定される。ただし、C++11では[`decay`](/reference/type_traits/decay.md)を適用するプ�セスが、C++14では下記`N == 2`の時のプ�セスが、それぞれ行われない。
...
- `N == 2` : `Types...`の１、2番目の型を`T1, T2`、`D1 = decay_t<T1>, D2 = decay_t<T2>`として
	- `T1,T2`に対する`decay`の適用は、少なくとも片方が恒�写像とならない（`is_same_v<T1, D1> && is_same_v<T2, D2> == false`となる）場合  
...
- `N >= 3` : `Types...`の１、2番目の型を`T1, T2`、残りのパラメータパックを`P...`とすると、`type = common_type_t<common_type_t<T1, T2>, P...>;`のように`type`を定義。
	- 先�2つの型の共通型を求めて、その型と3番目の型の共通型を求めて、その型と4番目の...というように再帰的に`common_type`を適用していく。


2つの型`T1, T2`に対する[`decay`](/reference/type_traits/decay.md)の適用がともに恒�写像となるのは以下の場合である。
...
- `T1, T2`の少なくとも片方はプ�グラム定義型に依�している
  - プ�グラム定義型とは、標準ライブラリの型を除いた、ユーザーによって定義された型のこと
	- 「プ�グラム定義型に依�している」とは例えば、プ�グラム定義型に対する`vector`�の特殊化のこと
- `T1, T2`に対する[`decay`](/reference/type_traits/decay.md)の適用はともに恒�写像となる（上記参照）
