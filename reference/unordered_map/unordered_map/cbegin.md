#cbegin
```cpp
<pre style='margin:0'><code style='color:black'>const_iterator cbegin() const noexcept;
</pre>
```
...
<code style='color:black'>unordered_map</code> は非順序連想コンテナであるため「先頭」に特に意味はないが、<code style='color:black'>cbegin()</code> で得られたイテレータを <code style='color:black'>[cend](/reference/unordered_map/unordered_map/cend.md)()</code> まで <code style='color:black'>operator++()</code> でイテレートすることで当該コンテナの要素を漏れなくダブりなく走査することができる。
...
```cpp
<pre style='margin:0'><code style='color:black'>{3rd,3}, {2nd,2}, {1st,1},</pre>
```
