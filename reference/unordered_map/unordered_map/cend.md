...
<code style='color:black'>unordered_map</code> は非順序連想コンテナであるため「最終」に特に意味はないが、<code style='color:black'>[cbegin](/reference/unordered_map/unordered_map/cbegin.md)()</code> で得られたイテレータを <code style='color:black'>cend()</code> まで <code style='color:black'>operator++()</code> でイテレートすることで当該コンテナの要素を漏れなくダブりなく走査することができる。
