...
unordered_map(const unordered_map& v);                              // (3)

unordered_map(unordered_map&& rv);                                  // (4)

explicit unordered_map(const allocator_type& a);                    // (5)

unordered_map(const unordered_map& v, const allocator_type& a);     // (6)

unordered_map(unordered_map&& rv, const allocator_type& a);         // (7)
...
- (2) の形式の場合、`value_type` は `*first` からこの `unordered_map` に EmplaceConstructible であること。

- (3)、(6)、および (8) の形式の場合、`value_type` はこの `unordered_map` に CopyInsertable であること。

- (4) の形式の場合、`allocator_type` のムーブ構築は例外終了しないこと。

- (7) の形式の場合、`value_type` はこの `unordered_map` に MoveInsertable であること。（但し、備考参照）


##効果

- (1)	バケット数最低 `n`、ハッシュ関数オブジェクト `hf`、キー比較用関数オブジェクト `eql`、アロケータオブジェクト `a` で、要素を持たない空の `unordered_map` を構築する。
	引数 `n` のデフォルト値は実装依存である。

- (2)	(1)と同様に `unordered_map` が構築された後、`[first, last)` の範囲の要素が挿入される。

- (3)	コピーコンストラクタ。`v` の全ての要素をコピーした、`unordered_map` を構築する。
...
- (4)	ムーブコンストラクタ。`rv` の全ての要素をムーブした、`unordered_map` を構築する。
...
- (8)	(2) の形式を `unordered_map(il.begin(), il.end(), n, hf, eql, a)` として呼び出した場合と同等である。
...
- (8)	(2)の形式を `unordered_map(il.begin(), il.end(), n, hf, eql, a)` として呼び出した場合と同等。
