...
`<unordered_set>` ヘッダは、�ーが要素である非順序連想コンテナを提供する。

[`<set>`](/reference/set.md) と異なり、各要素は�ーの順序ではなく�ーのハッシュ値に基づいて格納されるため、イテレータを用いたコンテナの走査の順序は（名前の通り）有意ではない。
...
`<unordered_set>` ヘッダでは、�ーの重複を許さない `std::`[`unordered_set`](unordered_set/unordered_set.md) クラステンプレート、および�ーの重複を許す `std::`[`unordered_multiset`](unordered_set/unordered_multiset.md) クラステンプレートを提供する。
...
| 名前 | 説明 | 対応バージョン |
|---------------------------------------------------------------|--------------------------------------------------------|-------|
| [`unordered_set`](unordered_set/unordered_set.md)           | �ーの重複を許さない非順序連想コンテナ(class template) | C++11 |
| [`unordered_multiset`](unordered_set/unordered_multiset.md) | �ーの重複を許す非順序連想コンテナ(class template)     | C++11 |
