...
| `pointer`        | 要素 `value_type`（`= Key`） へのポインタ型。スマートポインタも可であるが、通常は `value_type*`。<br/>標準では、`allocator_type::pointer`。<br/>しかし、本来は `std::`[`allocator_traits`](/reference/memory/allocator_traits)`<Allocator>::pointer` が正しいと思われる。<br/>（さもないと、必須である `allocator_type::value_type` のみを定義したユーザ定義のアロケータを使用することができないため） |
| `const_pointer`  | 要素 `value_type`（`= Key`） へのコンストポインタ型。スマートポインタも可であるが、通常は `const value_type*`。<br/>標準では、`allocator_type::const_pointer`。<br/>しかし、本来は `std::`[`allocator_traits`](/reference/memory/allocator_traits)`<Allocator>::const_pointer` が正しいと思われる。<br/>（さもないと、必須である `allocator_type::value_type` のみを定義したユーザ定義のアロケータを使用することができないため） |
| `reference` | 要素 `value_type`（`= Key`） への参照。<br/>標準では、`allocator_type::reference`。<br/>しかし、本来は `value_type&` が正しいと思われる。<br/>（さもないと、必須である `allocator_type::value_type` のみを定義したユーザ定義のアロケータを使用することができないため） |
| `const_reference` | 要素 `value_type`（`= Key`） へのコンスト参照。<br/>標準では、`allocator_type::const_reference`。<br/>しかし、本来は `const value_type&` が正しいと思われる。<br/>（さもないと、必須である `allocator_type::value_type` のみを定義したユーザ定義のアロケータを使用することができないため） |
