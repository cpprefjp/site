...
- [LWG Issue 811. `pair` of pointers no longer works with literal `0`](https://cplusplus.github.io/LWG/issue811)
    - C++11で、(5)のコンストラクタが、`U`が`first_type`へ、`V`が`second_type`へそれぞれ暗黙変換可能な場合にのみオーバーロード解決に参加するよう制約された
