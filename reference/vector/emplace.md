...
- C++14 : 例外が発生した場合、末尾への挿入であれば副作用が発生しない。（コピーコンストラクタ、ムーブコンストラクタ、代入演算子、ムーブ代入、および、`InputIterator`操作で例外が発生した場合を除く。）もし、非`CopyInsertable`な型`T`のムーブコンストラクタで例外が発生した場合、副作用は未規定。
